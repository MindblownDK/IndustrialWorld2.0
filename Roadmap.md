...
| Fluids / gases | 🛠️ WORKING ON | Pipe-gated transfer exists. **7.8.1-dev** hardens dense grid/ground pipe performance and enforces strict direct pipe joins after the reported 5-FPS/off-plane-link failure; Unity validation is pending. |
...
| UI / UX | 🛠️ WORKING ON | Runtime crisp UI scaling, responsive machine panels, build-wheel fit, recipe validation tooling, drop-limit notifications, inventory-weight readout, and terminal search are active. **7.6.0-dev** adds bottom-left and cockpit gravity-pull telemetry; **7.6.1-dev** restyles both as restrained LCD field instruments; **7.7.0-dev** reduces the on-foot footprint while preserving readable segmented surface telemetry, Unity-validated by Thomas. **7.8.0-dev** adds a conditional cockpit coast-path readout, Unity-validated by Thomas. **7.8.1-dev** removes destructive Grid Battery UI refreshes that flashed mode controls, pending Unity validation. |
