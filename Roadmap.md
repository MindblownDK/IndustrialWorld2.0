...
**Roadmap Version:** `9.21.5-dev`  
**Date:** 2026-08-30
**Status:** Celestial Ruins & Themed Worlds COMPLETED (6.32.0–6.36.3) — 33 themed ruins across 11 planets, slot-chest loot, breakable/salvage, per-block colliders; Crusader Ruins Premium (6.30.0); Offline Survival (11.4) Completed + Grid Gravity Fixed (6.27.3) + Passive Oxygen (11.5); World Management (11.7) Completed; Power, Vehicles & Combat (4.7.0) remains WORKING ON — base defense network + ammo logistics + status UX shipped (6.57–6.68), jetpack fuel (6.72–6.77) + Armor Stations and timed upgrades Unity-validated (6.80.0–6.80.4-dev); 6.80.3-dev hardens mountain footing and dry spawn safety; 6.80.5–6.80.6 maritime power/ports/docking/tank recovery are Unity-validated; 7.0.0-dev intentionally removes Encased Chain Drive content; 7.1.0–7.3.2-dev vehicle/power/armor UX is Unity-validated by Thomas; 7.4.0-dev starts active vacuum life-support foundations; 7.4.1-dev adds powered/hydrogen autonomous space dampers, Grid Battery persistence, and compile recovery pending Unity validation; 7.5.0-dev profile-driven atmosphere-to-space flight foundation is Unity-validated by Thomas; 7.6.0-dev adds shared gravity-pull telemetry; 7.6.1-dev replaces its generic rounded treatment with fitted LCD-style field instruments; 7.7.0-dev compact gravity LCD and sparse vacuum starfield are Unity-validated by Thomas; 7.8.0-dev cockpit orbital coast-path flight computer is Unity-validated by Thomas; 7.8.1-dev hardens battery/pipe regressions; 7.9.0-dev unifies cockpit, vitals, held-item, and hotbar screens under fitted LCD instruments while restoring strict five-cell same-plane pipe runs and conduit-volume placement protection; 7.9.1-dev replaces the initial inventory treatment with a cargo-matrix LCD terminal and matching crafting/production/recipe workstations; 7.9.2-dev keeps all equipment modules open, restores Grid Battery field/dock charging, adds cockpit full-velocity dampener hold and persistent third-person orbit, localizes pipe visual work, and rebuilds Ship Control as a matching LCD console; 7.9.3-dev restores the cockpit dampener state compile contract; 7.9.4-dev corrects body-origin water FX/math; 7.10.0-dev adds rare infinite Pirate World oil nodes, relic-gated high-power Jack Pumps, expensive Assembler crafting, research, and Pirate ruin loot; 7.10.1-dev repairs discoverable finite crude seeps, legacy planet configuration, loaded-chunk retrofit, and the infinite-extraction gate; 7.10.2-dev distributes finite seeps across selected oil-rich worlds while locking infinite Jack Pump nodes to canonical Planet_Pirate, Unity-validated by Thomas; 7.11.0-dev replaces the external ocean integration with native spherical water, radial boat wakes, repaired finite/infinite pool pumping, pipe-aware buffers, and dense slow-sinking crude; Thomas confirmed the native stack renders in Unity and reported terrain/cave/LOD follow-ups, addressed in 7.11.1-dev through 7.11.8-dev (authored planet scale, automatic solar system/asteroids, real ocean basins with ocean LOD, full sampled planet LOD, wrapped terrain/grass, active-world mining, dirt drops, grounded oil seeps, HUD/LOD interaction cleanup, and Unity compile recovery). **7.11.9-dev** additionally hardens padded spherical chunk coordinates, pooled generation/water queue epochs, sealed surface crust/caves, exterior-only separated trees, oil-site terrain caps, high-density full-planet LOD, late-viewer anchoring, automatic `System_Sol` library assignment, and visible deterministic asteroid proxies. **7.11.10-dev** restores the frame budget with bounded local chunk radii, capped proxy LOD, throttled water/scatter/grass work, and a density-selected one-location spherical spawn instead of visible dry-land relocation hopping. **7.11.11-dev** resolves the PlayerSpawner local-scope compiler recovery without changing that behaviour. **7.11.12-dev** bounds spherical generation/meshing/collider work, localizes voxel-water simulation, and restores a detailed orbit surface proxy without dense chunk streaming. **7.11.13-dev** fixes degenerate terrain collider spam, restores voxel inspection/mining fallback, wraps grass radially, restores continent/mountain terrain, and bridges legacy water/oil/drop configuration. **7.11.14-dev** upgrades LCD screen animations across all HUDs/terminals (animated scanline shimmer, phosphor boot sequences, Tektronix bezel corner brackets, pulsing status indicators) and scales voxel density by 32x to eliminate contour terracing/rings on planet surfaces. **7.11.15-dev** adds interplanetary space flight/transitions, strict planet-to-biome filtering, near-camera full-sphere LOD clipping, puddle-to-reservoir oil seep funnels, ruined Pirate Jack Pump salvage landmarks, top-right LCD inspection HUD styling, bare-hand tree harvesting, and fixes UI Toolkit scheduler casing. **7.11.16-dev** eliminates coastal water pull-down and inter-chunk step seams in `WaterMeshBuilder.cs`, retains coastline triangles in `PlanetOceanLodRenderer.cs`, relocates `WorldInspectionHud` back to top-left per feedback, and guarantees voxel material name resolution. **7.12.0-dev** introduces hierarchical multi-body sky orbits (planets orbiting Sun, moons orbiting planets, sub-moons orbiting moons) and Roadmap Era 4 3D Asteroid Belt zero-g procedural voxel generation. **7.12.1-dev** removes artificial warp prompts in favour of pure ship-driven interplanetary flight, ensures sparse 3D Asteroid Belt voxel spawning without surface shells, removes camera-following visual asteroids, and corrects the asteroidal material enum catalogue. **7.13.0-dev** delivers REAL SPACE: true Keplerian orbital mechanics (elliptical orbits, vis-viva velocities, T = 2π√(a³/μ)) for every planet/moon/sub-moon, a floating-origin + reference-frame engine (SpaceOrigin) that re-bases the scene for infinite continuous flight with zero warps, N-body gravity with genuine deep-space zero-g, per-body streaming re-targeting with per-planet chunk persistence, real scene geometry for every body (true LODs up close, sky projection far away), deep-space procedural minable asteroids outside planet/moon orbits, cosmic save/restore for orbital logouts, and the Warp Drive grid block as the ONLY warp (chargeable, power-hungry, research-gated via Setup Step 50). **7.13.2-dev** unifies the LCD theme across menus/settings/storage/ship terminal + compiles clean. **7.13.3-dev** declutters the HUD (compact vitals), eliminates HUD/panel overlap (HUD modules yield to panels), makes every heavy machine panel scrollable so slots always fit, adds machined panel depth, and fixes the boot-sweep scheduler. **7.13.5-dev** fixes the spawn launch (frame pinned to the home body), stops the LCD boot replaying on UI refreshes, removes the weather indicator, aligns vitals text, and compacts the gravity instrument. **7.13.6-dev** eliminates the through-the-planet launch entirely: linear interior gravity falloff (no core-escape energy), a 55 m/s radial fall-speed cap, poisoned-save rejection, rest-at-spawn, a 3 s spawn-grace on frame switches, and floating-origin tracking of the real player. **7.13.7-dev** kills the death loop (respawn destinations validated — no more respawn-in-space; 24 m/s survivable fall cap; spawn fall-damage grace), makes HUDs fade instantly on UI open, and renders the WHOLE active planet at high LOD (10k–163k verts, built progressively, upgraded on frame entry). **7.13.8-dev** fixes the remaining respawn sideways launch: the frame is pinned to the destination body before teleport with auto frame-switches suppressed until handover (teleports/warp/restore are delta-free), streaming is force-retargeted, and space bed/cryobed spawns are officially supported. **7.13.9-dev** removes the phantom constant X/Z pull: the floating-origin anchor tracks the frame body's true Keplerian position (no more orbital ground slide) and gravity is frame-relative (the frame body's own orbital acceleration is cancelled, so the sun/planets no longer pull the player sideways). **7.13.10-dev** anchors the sky to the active body (planets no longer follow the player), enforces ONE sun (quasar disabled), adds a warning/heat-ramp solar hazard instead of random sun death, and makes every planet SOLID with LOD safety colliders plus a larger, faster-streaming terrain bubble (no more flying through planets). **7.13.11-dev** closes the collision gap entirely (the shell hugs the real sampled terrain, stays on until streamed colliders exist under the player, bigger 5–8 chunk real-voxel bubble, 4-chunk collider radius) and makes crude-oil seeps fully visible: solid oil-soaked bore + reservoir rendered by the terrain mesh with idempotent write-retry, while the surface puddle stays a pumpable liquid. **7.13.13-dev** moves the block-info HUD to the top-left, fixes blank inspection targets (safety-shell colliders are now skipped by interaction rays), streams real terrain around the approach point when descending to a planet from orbit, enforces a 2,000 km planet-gap floor, and makes the sun a real visible mesh with auto-scaled warning/lethal hazard zones. **7.13.14-dev** lets the player actually LEAVE a gravity well (frame release at pull < 0.014 m/s² + crossover switching instead of the 1.25× trap), keeps other planets and the sun visible through the whole approach (far clip now tracks the system up to 50,000 km, true-LOD window 800 km), hides the fake sun sprite when the real sun renders, and thereby makes orbit-approach terrain generation engage properly. **7.14.0-dev** delivers REAL PLANET SURFACES: sampled terrain on every sky proxy (same density field as the voxel generator), a distance-based LOD ladder for ALL bodies (642 → 163k verts as you close in), proxy→true-position convergence with a crossfade hand-off, and the true-LOD window raised to 2,500 km so the approached planet's real surface is visible for the whole interplanetary crossing. **7.14.1-dev** recovers the CS1061 `Color.rgb` compile error in the sky-proxy tint path. **7.15.0-dev** replaces the sampled impostor with REAL voxel planet surfaces: every body generates its surface as actual voxel chunks at LOD levels (far 128–512 m whole planet within 8,000 km, mid 32–128 m whole planet within 150 km, near 8 m ring below 4 km altitude, 1 m gameplay bubble unchanged), the impostor becomes a 1–3 s bridge until the real surface is ready, the minimum planet gap rises to 8,000 km (vast interplanetary space), and `SurfaceNetsJob` is voxel-size parameterised. **7.15.1-dev** recovers the CS8773/CS8983 C# 9 struct field-initializer compile error in `SurfaceNetsJob`. **7.16.0-dev** delivers ONE rendered surface at every distance (strict LOD nesting with the L0 bubble's true stream centre, FAR stepping aside under meshed MID chunks — no more ghost surface above the terrain), solid planets on approach (safety shell built for every body, upgraded on frame entry — no more flying through), and oil fields visible in the LOD levels (deterministic site map matching the decorator's cell hash: puddle disc, tapered bore, reservoir sphere — sampled by the LOD chunks). **7.17.0-dev** makes EVERY planet active at all times (real-voxel window raised to 60,000 km covering the whole system, far clip 80,000 km), adds forge-forward body registration (registry bodies added after bootstrap — e.g. World Forge planets — automatically spawn real voxel worlds), and recovers the NativeParallelHashMap `.Length`/`.Count` compile errors in the oil-site path. **7.17.1-dev** recovers chunk generation: the unconstructed `oilSites` job container broke ALL terrain (gameplay world + LOD) — every scheduler now passes a constructed map, which restores spawn terrain, load stability (falling-in-space fixed via dominant-frame restore), and the world-space HUD position; load restore no longer forces deep space on a missing frame name. **7.17.2-dev** fixes the HUD being entirely outside the screen view on non-1080p windows (runtime panels now use ScaleWithScreenSize instead of ConstantPixelSize — the HUD fits any window) and makes the LOD levels nest with EXACT abutment (no overlap bands where the coarser LOD surfaced above the real terrain). **7.17.3-dev** returns ALL UIDocuments to Screen Space (they were serialized as 1920×1080 world-unit quads — the camera saw only the center slice, cutting the HUD off at the screen edges), makes the LOD inner nesting STRICT on eviction too (the 8 m ring no longer trails the player above the 1 m terrain), and adds real mesh colliders to the 8 m NEAR ring (walk on real voxel terrain from the 1 m bubble edge to ~2.4 km; 1 m collider bubble raised to 6 chunks). **7.17.4-dev** adds a 4 m DETAIL ring (900 m) between the 1 m bubble and the 8 m ring and widens the 8 m ring to 3,000 m (shell-filtered rings = bigger radii, fewer chunks), forces ALL UIDocuments to Screen Space Overlay at runtime (HUD can never be pushed off-screen again) with width-priority scaling (HUD stays large on wide windows), and fixes the MeshCollider 'three distinct vertices' error on empty ring meshes. **7.17.5-dev** recovers the CS1061 `UIDocument.renderMode` compile error — the ScreenSpaceOverlay force now uses `PanelSettings.renderMode` via the shared `ApplyUiScaleAndFit` (single source of truth). **7.17.6–7.17.9-dev** polish grass banding, LOD nesting/deflation, interplanetary voxel streaming, UI scale, and NaN sun-visual rejection. **7.18.0-dev** delivers planet-specific sky art (per-world zenith/horizon/sunset/aurora/dust palettes, themed sun/fog/ambient, atmosphere-rim colours) and deep-space nebulae, authored non-destructively through Setup Step 51. **7.18.1-dev** guarantees the authored radial horizon owns the camera background, corrects horizon-plane colour and local night grading, and makes Step 51 preserve a standalone-build shader reference, Unity-validated by Thomas. **7.19.0-dev** adds eclipse-aware procedural solar glare, palette-tinted lens ghosts, and bounded motion-readable vacuum dust through the expanded non-destructive Setup Step 51. **7.20.0-dev** delivers FULL-PLANET REAL VOXEL COVERAGE: the active body streams its whole surface as real voxel chunks (budget-bounded FULL shell, 8–16 m on home-sized worlds) plus a new 2 m detail ring and coverage-driven 4 m/8 m rings, driven by a new `fullVoxelRadiusKm` world setting (default 50 km — the whole planet on typical worlds); chunk generation is ~10–30× faster via per-chunk surface-column caching, streaming budgets are raised, and candidate scans are cached; and a proximity-hold frame failsafe guarantees the scene frame (and therefore real voxel terrain) always engages on arrival at non-starter planets and small moons whose gravity can never dominate the star — the "only LOD, no surface" interplanetary bug is fixed. **8.0.1-dev** recovers the flat-world removal compile errors (CS0103 `flatWorld`, CS8321 dead local). **8.0.0-dev** REMOVES THE FLAT WORLD entirely (`VoxelWorld`, flat `ChunkGenJob`/`ChunkHeightJob`/`NoiseUtility` and every reference) — the game is spherical planets only — and namespaces the home-body save key so old flat-era chunk saves can never load into the sphere (they were rendering as stacked flat terrain layers with gaps inside the planet). This is a MAJOR bump: removed core system + home save key change (home planet regenerates fresh). **8.0.2-dev** restores the spherical surface with gradient-corrected chunk columns (the 7.20.0 centre-only column made LOD chunks render as flat slabs — the real cause of the "layers of flat terrain" planet). Unity validation of 7.19.0/7.20.0/8.0.0–8.0.2 remains open. **9.0.0-dev** REBUILDS PLANETARY GENERATION as a GPU-driven engine (Rework Phase 1): compute-shader density evaluation (64³-cell nodes, AsyncGPUReadback), Dual Contouring with QEF vertices (fewer polygons, sharp ridges), and an asynchronous spherified quadtree (6 cube faces, Burst background jobs) — one unified `PlanetField` (HLSL ⇄ C# lockstep) shared by the GPU surface AND the 1 m gameplay bubble, watertight by construction (ghost-cell stitching + owner-edge quads + LOD skirts), plus a quadtree ocean sphere with boat wakes and a reserved flow-map channel; the legacy impostor/voxel-LOD/ocean-LOD path is deleted. Phase 2 (gameplay bubble on the new engine, edit-delta persistence) and Phase 3 (liquid-flow simulation rework) follow. Fresh save required; 9.0.0/9.0.1 Unity-validated by Thomas (runs great, startup load only). **9.1.0-dev** completes Rework Phase 2 as the SINGLE-SURFACE HANDSHAKE: the bubble publishes its meshed+collider ball, the GPU LOD skin clips its fragments inside it (mined tunnels show real voxels, never phantom terrain), LOD colliders yield to bubble colliders so tunnels dig freely, and fully-swallowed nodes hide (no near-field shimmer) — the planned 64³ bubble/eDelta rewrite was made unnecessary by the unified field. **9.2.0-dev** lands the field-report fixes: handshake engagement (deep mining restored), all-depth approach colliders (no more flying into planets), shared radial lattice (watertight node borders planet-wide), TRUE-DIRECTION sky proxies (the dot you fly toward IS the planet — gravity/atmosphere engage on arrival), body-anchored world spawn (no more respawning in space), and open-space minable asteroids (deep space + high orbit). **9.3.0-dev** closes the second field report at root-cause level: the SRP-Batcher-broken bubble cutout (the real 0.5 m mining wall AND the look-down terrain flicker), LOD-skin radial deflation, GLOBAL analytic radial band (gap-proof, planet-wide shared lattice) with split hysteresis, a high-speed depenetration guard, NaN-proof self-reporting Keplerian orbits (missing planets), self-healing world-spawn respawn, the asteroid km/m unit fix, and a procedural burning STAR surface (granulation, starspots, limb darkening). **9.4.0-dev** deletes the fake sky-proxy system entirely (REAL-ONLY space: real planets/moons/star/asteroids + backdrop starfield/nebulae), fixes the ocean-killing splitSet job exception, adds NaN defence-in-depth with self-naming diagnostics across the cosmic chain, and makes world-spawn respawn reject un-anchored legacy points outright (self-healing saves). **9.5.0-dev** replaces the chunk-centre linear surface approximation with an exact per-chunk surface LATTICE (Burst prepass + trilinear interpolation) — voxels now generate the same field the GPU renders (no hollow planets, no generation-time gaps, mine to the core), removes the duplicate auto sub-moons, adds honest TRUE-POSITION beacons for distant bodies, makes respawn deep-space-proof via analytic PlanetField spawns, and instruments every planet engine with 15 s telemetry. **9.5.1-dev** adds the footing-based collider yield (mining unblocked for good), the world-aligned 8 m lattice (no between-chunk seams), full-moon-sized navigation beacons, and calm-shore open-ocean water. **9.5.2-dev** makes the near-player LOD collider yield UNCONDITIONAL, adds self-naming mining diagnostics (any zero-effect swing prints the exact blocker), converts respawn to spherical parking with staged logs and a final analytic hard clamp, and raises streaming budgets. **9.5.3-dev** (diagnosed directly from Thomas's logs) rejects stale pre-9.5 chunk saves via region format V3 (mixed-era worlds self-heal — fixes the unmineable phantom surfaces AND the floating-island respawn), routes the death screen's World Spawn through the body-anchored healing respawn, and unifies saves into one folder per world (`Bodies/<planet>` subfolders, auto-migrated). **9.5.4-dev** fixes THE mining bug at its true root (chunk MeshColliders never re-baked after remeshes — reference-equality guard on a reused Mesh object), restores full stats on world-spawn respawn, holds respawn handover until real ground collision exists, and grounds on-foot load positions that drifted above their planet. **9.5.5-dev** adds border write-through for edits (padded neighbour copies update on boundary digs — the last mining ghost wall, corners included) and teleport immunity for the depenetration guard (no more mid-spawn snaps poisoning saves). 9.0.x–9.5.x core engine confirmed stable by Thomas (mining, respawn, saves, planets, asteroids all field-validated). **9.6.0-dev** delivers Rework Phase 3 — LIVING WATER: seed-derived mountain springs with budgeted infinite-source refill feeding real downhill streams through the planet-aware cellular sim, plus live ocean currents (divergence-free noise gyres + along-shore coastal flow) in the GPU ocean's UV2 flow channel. **9.7.0-dev** completes the living-water look: liquid surfaces drape onto the smooth terrain crossing (no more voxel-quantised 'tarp' sheets — applies to water, streams and oil), natural lakes fill genuine terrain dips at generation, and crude-oil seeps get real 1-voxel basins (small dark ponds). **9.7.1-dev** bans solid-oil air-writes (the floating black 'tarp' was the bore poking out of slopes — it now recolours existing rock only), rebuilds the liquid drape on trustworthy below-cell field densities, and makes lakes 3× more common with placement logs. **9.7.2-dev** rebuilds the oil seep as CARVE-AND-FILL: a real crater dug into the terrain, oil only inside the carved cells (hovering geometrically impossible), soaked dark floor revealing the shaft mouth down to the reservoir. **9.7.3-dev** fixes the true seep culprit — RoundToInt(up) produced DIAGONAL carve steps on the sphere (slanted see-through holes, scattered fill): basins now use one dominant cardinal axis with voxel-scanned ground and a level pond capped at the centre ground line. **9.7.4-dev** sinks the seep pond surface into its crater (fill stops one cell under the ground line), makes the shaft core and reservoir interior LIQUID crude inside soaked-rock casings (pumpable pond→reservoir column), and gates generated sea water by the continent mask (no more hidden water lenses under beaches; ocean patches match on near tiles). **9.7.5-dev** is the reservoir-loop compile recovery (CS1023/CS0103 — misplaced declaration between stacked for-loops). **9.7.6-dev** adds radial liquid leveling: liquid corner vertices average world RADII and solve their height onto that shell — ponds/lakes/oil sit level against gravity at every latitude with smoothed edges, while sloped streams keep following terrain. **9.7.7-dev** builds seep ponds from constant-RADIUS shells (level by construction), always remeshes padding-touched neighbours (no more crater slivers), and corrects the shoreline water gate to chunk-centred ocean depth. Phase 3 living water is Unity-validated by Thomas (streams, lakes, level radius-shell oil seeps, dry beaches). **9.8.0-dev** opens Phase 4 with STARLIGHT: true star-direction lighting from the viewer's cosmic position (surface/orbit/deep space), atmosphere-to-space lighting blend, star-glow-tinted daylight, deep-space hard light + near-black ambient, and a water material glow-up. **9.9.0-dev** delivers Beauty Pass Part 2 — LIVING GROUND: distance-faded procedural detail with real detail normals, ~55 m macro colour variation, a wet glossy waterline band, and gusty wind-swept grass (denser, taller, meandering direction). Next: Phase 4 Part 3 — sky/atmosphere polish; then Phase 5 (black hole & quasar) (terrain material beauty, real grass, stunning water, sun-driven world/space lighting); **Phase 5 celestial monuments: BLACK HOLE and QUASAR bodies (real, flyable-to, lethal)**. **9.10.0-dev** SHIPS THEM: real black hole + quasar bodies seeded far beyond the planets (true positions, inverse-square N-body pull, flyable event horizons, Doppler-beamed accretion discs, photon rings, quasar relativistic jets with shear damage, crush death at the horizon, honest true-direction beacons with warp-drive singularity lock, one-sun enforcement + solar corona polish, non-destructive Setup Step 52). **9.11.0-dev** turns the black hole into an ENDGAME RESOURCE NODE: the Singularity Harvester grid block (contained mini black hole, spinning accretion disc, glowing containment coils) harvests Singularity Matter within 2,500 km of a horizon — yield climbs with proximity, quasar pays 1.5× — and auto-pushes to grid cargo; resource + prefab + item + recipe + tier-8 research authored via non-destructive Setup Step 53; Singularity Matter is earmarked for the future STAR CRAFTER (planet/star-system authoring). **9.12.0-dev** fixes the field-reported remnant visibility bugs (converging beacon projection — no more 'running away' beacon; guaranteed far-clip coverage + a seamless 50,000–62,000 km beacon-to-real-geometry crossfade) and adds REAL GRAVITATIONAL LENSING (bent-light photon ring hugging the horizon where the disc plane crosses). It also splits the harvest: black holes shed rare ANTIMATTER, quasars shed rare DARK MATTER — both containment-class (plain cargo refuses them; the new armoured CONTAINMENT VAULT grid block stores them, portable Antimatter/Dark Matter CANISTERS carry them, non-destructive Setup Step 54 authors the whole chain). **9.12.1-dev** is the Unity compile recovery (CS0104 Random ambiguity, UAC1001 serialization). **9.13.0-dev** turns the vault into a POWERED containment system: field pressure simulation (stable band, critical state, annihilation at zero pressure — power loss on a loaded vault is an emergency), a cockpit alert banner while piloting, a flagship machine panel with a live black-hole visual and pressure gauge, and a grand full-cell block (four field pylons, contained black hole with spinning disc, counter-rotating containment rings, live status light; 24 slots / 120 t); it also fixes the VoxelTerrainEnhanced _VoxelSeaRadius compile error; **9.13.1-dev** is the vault-panel compile recovery (panel-attached guard). **9.14.0-dev** fixes the magenta horizon shader (HDR/LOD combo removed), makes raw exotic matter UNCARRYABLE (pressurized canisters only, with decaying field pressure in the player inventory, a pocket pressure HUD, and fatal collapse at zero — 'KILLED BY CONTAINMENT COLLAPSE'), adds the STAR LOCATOR grid block (AUTO/SPECIFIC targeting of black hole/quasar/sun/planets/moons with a true waypoint marker the warp drive locks onto — Setup Step 55), grows the harvester and vault into full-cell monuments with power-driven animations, gives the harvester its flagship machine panel, and re-centers the vault panel's black-hole visual. **9.14.1-dev** fixes the field round: the harvester/navigator right-click panels now open (GridBlockHasUI whitelist), the pink horizon/disc materials get force-rebound shader references from Steps 53/54, the vault panel's black hole is perfectly centered, live panel updates no longer auto-scroll, vault warnings only reach the pilot of the vault's own grid, and the block is renamed to the ASTRAL NAVIGATOR. **9.15.0-dev** closes the 'proper asteroids & planet visibility' roadmap item: planets and moons are now visible from ANYWHERE in the system as real shaded world discs (new DistantPlanet shader — sun-lit day side, terminator, starlit night side, atmosphere rim), the beacons use the converging projection pinned at 62,000 km with a distance-based 60,000–80,000 km crossfade to the real geometry (no far-clip culling, no invisible gap), and the asteroid fields become proper belts — double density over a 14 km ring, cluster families with shared ore, slow drift + tumble, 8–140 m rocks. Both exotics are required for the future Star Crafter / World Engine. **9.16.0-dev** OPENS THE LIQUIDS OVERHAUL (Part 1 of 3): all 7 liquids are now placeable in the world with real per-liquid looks and physics — five new save-compatible fluid materials, per-liquid flow (viscosity, spread, real density layering: fuel floats on water, water on crude), 7 mesh submeshes with 7 shader profiles (foamy water, glossy black crude, rainbow-sheened refined/fuel/MGO, matte HFO, glowing coolant; thin-film iridescence + emission added to VoxelWaterURP), industrial-world fuel/refined-product lakes (BodySettings.industrialWorld + Step 56 flagging), the UNIVERSAL Liquid Bucket (scoops/places all 7, fills/pours into liquid tanks) — RETIRED in the same version's field round and replaced by the LIQUID CANISTER (10 L, one liquid: RMB scoops 0.5 L/click from pools until full, LMB pours 0.5 L/click into the world, RMB exchanges with liquid tanks, water/marine pumps and the infinity jack pump — non-destructive rename/migration, old saves keep working), and the fake quadtree ocean shell is REMOVED — oceans are now generated voxel basins below sea level like everything else (GpuOceanEngine self-disables if a legacy scene still carries it). Flow remake included: mining no longer destroys surrounding water (legacy dry-cave prune removed), the sim runs at 10 Hz with an 8-chunk budget and faster spread steps, and every terrain edit wakes the affected chunks with an immediate fluid step so water reacts the same frame — instant-reaction flow. Field round included: solid fluid-soaked rock (oil bore/reservoir casing) is preserved by the sim and always diggable, water pours at half-cell ticks so excavation under liquid stays readable, and liquid surfaces gained a procedural texture layer (fine ripples, colour patchiness, per-liquid sparkle). Part 2 SHIPPED in the same version — the full FIRE system: per-liquid flammability (fuel fastest, HFO crawls; water/coolant quench), fires that consume the fuel they sit on, neighbour spread with a bounded cap, the Igniter tool (Step 57) plus Ifrit fireball/fire-wall splash ignition, procedural FireURP flames on the liquid surface with a pool of 6 flickering point lights, and armour-mitigated burn damage (12 DPS) for players in the flames. Fire is transient — nothing is saved. Part 3 SHIPPED in the same version — PER-LIQUID PLAYER PHYSICS: PlayerWaterState now reports the dominant liquid at the body; oils drag and sink (crude 0.40x + 1.05 m/s sink, HFO 0.30x near-molasses, refined/MGO milder), liquid fuel floats you while its caustic DoT (armour-mitigated ApplyCaustic) eats your skin, hot coolant scalds with the armour-escalating burn, every liquid blocks breathing, and the underwater camera gets per-liquid looks (black crude murk with a 12 m view, glowing teal coolant, amber fuels/oils). THE LIQUIDS OVERHAUL IS COMPLETE (Parts 1-3 + all field rounds). Field round 12 in progress: the material-registry regression that anonymized inspection-HUD material names is fixed three ways (built-in runtime fallback registry, HUD name fallback, Step 57 asset self-repair). The top-left inspection HUD's stuck-_visible reload bug is fixed (remount state reset + orphan self-heal), and the top-right block display (BlockRotationHud) got the same hardening plus a ground-truth inventory fallback. Water: the console log proved 0 generated water voxels near spawn while seaRadius(8096) sits 4 m BELOW the mean surface (8100) — the water diagnostics now also evaluate the planet's own noise field (oceanCols/1024) + a wide ±20-chunk generated-chunk scan, delivered as an on-screen toast, to settle whether the planet's field produces ocean basins at all before the generation fix. Next roadmap item after this field round: the surface-texture pass, then the rest of the roadmap. **9.17.0-dev** delivers that SURFACE-TEXTURE PASS: both terrain meshers (`SurfaceNetsJob` + `GpuDualContourJob`) now write each surface vertex's dominant material id into the vertex-colour alpha (a channel nothing read, never persisted — save-compatible and non-destructive), and a new shared `VoxelSurfaceTextures.hlsl` included by BOTH terrain shaders (`VoxelTerrainEnhanced` — the live material and LOD-skin source — and `VoxelTerrainURP`) renders per-material procedural textures: stone strata/hairline cracks, wind-rippled sand with bright crests, glinting ore flecks tinted by each ore's own colour (uranium breathes faint green), matte coal streaks, wet-gloss oil-soaked rock, vertical wood grain, clumped grass with dry patches and blade streaks that shed to grained soil on steep slopes, faceted ice/crystal, columnar basalt with faint warm veins, sickly gloss-pooled acid bog, martian dust drifts, and a restrained generic grain for unknown/custom materials — every class also emits tangent-plane relief gradients so ripples, cracks and facets genuinely catch the sun. Cost is budgeted (≤3 two-octave noise evals + one 3×3 voronoi for crack/facet classes — cheaper than the retired 9.9.0 micro-detail), fully camera-distance faded, and dialled by a single `_SurfaceTexStrength` material property (default on). No prefabs/items/recipes/research are added — no setup step required. Pending: Unity validation of 9.17.0 plus the still-open 9.16.0 water-diagnostics toast; then the rest of the roadmap. **9.17.1-dev** is the field-round compile recovery: 9.17.0's uint material-id path broke both terrain shaders at Unity's default legacy compilation level (integer types unsupported → pink terrain); ids are now pure float end to end, and an offline rig compile-validates all 12 pass/entry-point combinations of both terrain shaders plus the shared include across all 29 material ids (12/12 PASS). Unity validation of the surface-texture pass remains open. **9.17.2-dev** is the second field-round compile recovery: the sand-ripple vector×matrix expression Unity's d3d11 compiler rejected is now a single dot product, and all three shader sources are stripped to pure ASCII (zero non-ASCII bytes — nothing for any import lexer to choke on); 13/13 offline compilation units pass. **9.18.0-dev** is the field round Thomas's report triggered — the GPU grass field never actually showed: Low-tier quality returned density 0 (grass silently off), the single RenderMeshInstanced call exceeded the 1023-instance limit and threw every frame on higher tiers, and the blade was a flat 2-triangle quad. Now: real tapered curved blade meshes (4 levels, pointed tip, baked lean), height-squared wind arcs along the blade facing, rounded viewer-aware shading, root AO, per-blade hue variation, real main-light shadows, an 8 m edge fade, batched 1000-instance draws, body-local anchors re-projected on floating-origin rebases, a Low-tier quality floor (sparse but visible), raised density defaults, and a one-line `[Grass]` rebuild diagnostic. Unity validation pending (Thomas's console line + meadow screenshots). **9.18.1-dev** is the CS1061 compile recovery (`body.localToWorldMatrix` -> `body.transform.localToWorldMatrix`, both call sites; all other `body.` usages re-verified against CelestialBody's real members). **9.18.2-dev** is the second grass field round: blades are PLANTED (the 6 cm surface-sample lift and 35 cm anchor lift are replaced by a 5 cm-below sample minus a 2 cm root tuck — no more floating), the black-blade bug is fixed (the shadow-coord attenuation path that zeroed outside regular mesh draws is replaced by translucent wrapped diffuse with a green ambient floor — a blade mathematically cannot render black), mining now removes grass (new `SphereWorld.PlayerEditVersion` bumped once per VoxelEditor edit batch; the renderer rebuilds within 0.6 s — deliberately NOT triggered by fluid-sim/generation writes to avoid rebuild storms), and the top-right card name no longer flickers away while mining (live inventory is now the primary name source with empty-string guards, asset-name fallback, and a 0.75 s anti-flicker holdover; panels still hide it instantly). Grass shader revalidated offline (vert+frag PASS, pure ASCII). Pending: Unity validation of 9.18.0–9.18.2 (Thomas's meadow screenshots + `[Grass]` console line), then the 9.16.0 water-diagnostics toast, then the rest of the roadmap. **9.19.0-dev** opens the WEATHER & CLIMATE foundations: the previously-dormant `VoxelEngine.Weather` stack is activated (`CosmosBootstrap` now creates the `_Weather` controller and applies per-body climate), made per-planet via a version-gated additive `WeatherClimateProfile` on `BodySettings` (precipitation mode, storm chance, sun-darkening, wind/fog/thunder), auto-disabled on airless bodies via `WeatherAllowed`, with a non-fighting sun-darkening/flash multiplier consumed by `SunLightController`, unified thunder (synced audio + lightning), and non-destructive Setup Step 58 that authors themed climate profiles (desert/ice/ocean/airless) and ensures the scene controller; **9.19.1-dev** is the first field round — rain now falls radially toward the planet core (spherical-world weather frame + local-space particle velocity instead of world gravity), rain renders as long thin dense streaks, and rain audio no longer mutes when walking (the under-roof probe samples along radial up instead of a world-Y voxel column) — Unity re-validation pending by Thomas. **9.20.0-dev** is the immersion round: a procedural two-layer cloud deck (new WeatherClouds + WeatherCloudsURP shader, coverage/drift/colour driven by the weather state, lightning-lit, yaw-free radial alignment), rain rendered through a dedicated fog-aware transparent particle shader (WeatherParticlesURP) with brighter denser streaks from a raised emission slab, softened storm fog, and a fully rewritten spatial WeatherAudio — wide stereo droplet-layered rain bed, real 3D metal/wood roof pattering from the actual shelter surfaces, speed-of-sound-delayed directional thunder, seamless crossfade loops; OnThunder now carries the strike world position. **9.20.1-dev** hotfixes a case-sensitive typo (`TransformObjectToHclip` → `TransformObjectToHClip`) in the two new weather shaders that made the cloud dome render magenta (the "pink circle") and the rain/snow particles render magenta/invisible. **9.21.0-dev** couples weather into wind: `WeatherManager.WindMultiplier` now drives wind-turbine output/degradation and the ambient `WindField` during storms. **9.21.1-dev** restores visible rain streaks and darkens the sky dome during storms via `_VoxelWeatherDarken`. **9.21.3-dev** rebuilds rain on the proven `SpaceDustRenderer` path (procedural billboard-UV streaks, no texture/Stretch) with a `[Weather] Rain:` diagnostic. **9.21.4-dev** adds unconditional `[Weather] Manager heartbeat` / `[Weather] Rain heartbeat` diagnostics to pinpoint whether weather is even active (and on which body) before touching rendering again. **9.21.5-dev** fixes the mixed-mode velocity-curve error that froze rain in the sky (all three axes now share one curve mode), and ducks the weather audio to ~25 % when a full-screen UI panel is open. Unity validation pending by Thomas.
...
| Domain | Status | Notes |
|--------|--------|-------|
| Voxel world / planet / gravity | ✅ COMPLETED | Planet terrain generation + radial gravity now validated. **6.27.3-dev** fixes grid gravity: dampeners no longer brake vertical fall when not in hover-hold (was causing slow-mo fall), center-of-mass set to average block position to prevent tipping, and `StabilizeGroundAlignment()` gently slerps grounded grids to planet up (4 Hz, only when landing gear locked / wheels grounded / low vert speed). **7.9.4-dev** corrects body-local water logic. **7.10.0-dev** makes very rare deterministic infinite crude nodes a Pirate World-only resource; **7.10.1-dev** adds reliable finite seep discovery and saved-chunk retrofit; **7.10.2-dev** distributes finite seeps across selected oil-rich worlds while retaining canonical Planet_Pirate-only infinite nodes, Unity-validated by Thomas. **7.11.0-dev** begins native spherical water/wakes and repaired pool pumping; Thomas confirmed the native stack renders, while terrain/cave/LOD follow-ups are addressed in **7.11.1-dev** through **7.11.9-dev**, including queue-epoch/padded-coordinate integrity, sealed surface caves/oil bores, exterior-only tree separation, automatic celestial recovery, the 7.11.10 bounded streaming/one-location spawn performance recovery, 7.11.12 bounded generation/mesh/water budgets with detailed orbital LOD, 7.11.13 valid terrain colliders, wrapped grass, all-tier voxel mining, rounded continent/mountain generation, and legacy water/oil/drop recovery, **7.11.14-dev** 32x sub-voxel density scaling that eliminates contour terracing/rings on planet surfaces alongside unified animated LCD screen HUD/terminal polish, | Voxel world / planet / gravity | ✅ COMPLETED | Planet terrain generation + radial gravity now validated. **6.27.3-dev** fixes grid gravity: dampeners no longer brake vertical fall when not in hover-hold (was causing slow-mo fall), center-of-mass set to average block position to prevent tipping, and `StabilizeGroundAlignment()` gently slerps grounded grids to planet up (4 Hz, only when landing gear locked / wheels grounded / low vert speed). **7.9.4-dev** corrects body-local water logic. **7.10.0-dev** makes very rare deterministic infinite crude nodes a Pirate World-only resource; **7.10.1-dev** adds reliable finite seep discovery and saved-chunk retrofit; **7.10.2-dev** distributes finite seeps across selected oil-rich worlds while retaining canonical Planet_Pirate-only infinite nodes, Unity-validated by Thomas. **7.11.0-dev** begins native spherical water/wakes and repaired pool pumping; Thomas confirmed the native stack renders, while terrain/cave/LOD follow-ups are addressed in **7.11.1-dev** through **7.11.9-dev**, including queue-epoch/padded-coordinate integrity, sealed surface caves/oil bores, exterior-only tree separation, automatic celestial recovery, the 7.11.10 bounded streaming/one-location spawn performance recovery, 7.11.12 bounded generation/mesh/water budgets with detailed orbital LOD, 7.11.13 valid terrain colliders, wrapped grass, all-tier voxel mining, rounded continent/mountain generation, and legacy water/oil/drop recovery, **7.11.14-dev** 32x sub-voxel density scaling that eliminates contour terracing/rings on planet surfaces alongside unified animated LCD screen HUD/terminal polish, **7.11.15-dev** strict planet-to-biome filtering, puddle-to-reservoir oil seeps, ruined Jack Pump salvage landmarks, full-sphere LOD clipping, bare-hand tree harvesting, and **7.11.16-dev** seamless level coastal water and inter-chunk water continuity without step seams. Full Unity validation is pending. |
| Cosmos / star system framework | ✅ COMPLETED | Active solar system navigation, high-orbit flight telemetry, and `CosmosBootstrap.Instance.TransitionToPlanet` are implemented (**7.11.15-dev**). **7.12.0-dev** introduces hierarchical multi-body sky orbits (planets orbiting Sun, moons orbiting planets, sub-moons orbiting moons) and Roadmap Era 4 3D Asteroid Belt zero-gravity procedural voxel generation. Players can warp or fly between planets, moons, and asteroid belts in the solar system. |
...
| Unified grid placement | 🟡 PARTIALLY COMPLETE | Detail/Structural placement, shape variants, screen sources, and unified Item/Gas/Liquid pipe placement/networks are validated. **5.69.0-dev** adds additive movable-grid save/restore for Structural and Detail blocks, variants, settings, and attached pipes; Thomas validated it in Unity. **7.5.0-dev** keeps ordinary blocks exactly lattice-centred when building from landing gear or any existing grid; Thomas Unity-validated it. Remaining positional-indexing work keeps this broader area partially complete. |
| Power (wind, hydrogen) | ✅ COMPLETED | Modular turbines are mature and production-ready for the current scope. |
| Fluids / gases | 🛠️ WORKING ON | Pipe-gated transfer exists. **7.8.1-dev** hardens dense grid/ground pipe performance; **7.9.0-dev** restores up-to-five-small-cell same-plane runs while retaining strict off-plane rejection and conduit-volume placement protection; **7.9.2-dev** replaces broad all-pipe visual invalidation with changed-corridor dispatch, a one-rebuild budget, hash-gated forced rebuilds, and targeted ItemPipe endpoint scans. Unity validation is pending. |
...
| Sky / atmosphere / space rendering | 🛠️ WORKING ON | **7.5.0-dev** delivers a real upper-atmosphere-to-vacuum handoff: profile-driven air density, dark space camera transition, altitude-aware far clipping, and stable camera-relative celestial bodies without a duplicate home-planet proxy; Thomas Unity-validated this foundation. **7.7.0-dev** adds a sparse, deterministic vacuum starfield that fades through the same transition, Unity-validated by Thomas. **7.13.0-dev** replaces the sky-proxy model with REAL space: every body is real scene geometry placed by the floating origin (true LODs within 200 km, compressed sky projection beyond), deep space renders true vacuum, and the starfield/sky transitions carry through the atmosphere handoff. **7.14.0-dev** makes every planet a REAL PLANET: sky proxies sample the same terrain density field (continents/oceans visible from space), all bodies run a continuous distance-based LOD ladder (642 → 163k verts), the proxy converges to the body's true position/size and crossfades into the real LOD (window raised to 2,500 km), and the display-colour wash is replaced by a subtle 0.18 tint so terrain always reads. **7.15.0-dev** delivers REAL VOXEL PLANET SURFACES: the whole planet is generated as actual voxel chunks at LOD resolutions (far/mid whole-planet + near 8 m ring + 1 m gameplay bubble), the sampled impostor is demoted to a 1–3 s bridge, planets are spaced 8,000 km apart, and the true-LOD window rises to 8,000 km. **7.16.0-dev** fixes the double surface (strict level nesting — one surface at every distance), makes planets solid on approach (safety shells built for every body), and adds oil-field rendering to the LOD levels (deterministic puddle→bore→reservoir sites sampled by LOD chunks). **7.17.0-dev** raises the real-voxel window to 60,000 km so EVERY planet is active at all times (far clip 80,000 km), and adds forge-forward registration so runtime-created bodies automatically become real voxel worlds. **7.18.0-dev** adds themed planet skies and deep-space nebulae; **7.18.1-dev** guarantees the planet-specific radial horizon replaces the default camera sky and survives standalone shader stripping through non-destructive Step 51 authoring, Unity-validated by Thomas. **7.19.0-dev** adds eclipse-aware solar glare/lens ghosts and bounded world-anchored vacuum dust; vacuum audio ducking remains the next open ambiance target. **7.20.0-dev** delivers full-planet real voxel coverage (whole-planet FULL shell + 2 m/4 m/8 m rings, `fullVoxelRadiusKm` world setting default 50 km), ~10–30× faster chunk generation (per-chunk column caching), raised streaming budgets, cached candidate scans, and a proximity-hold frame failsafe so non-starter planets and small moons always stream real surfaces on arrival. Unity validation of 7.19.0/7.20.0/8.0.2 remains open. |
| Gravity / orbits | 🛠️ WORKING ON | Grid gravity uses radial `GravityProvider` + `GetSurfaceRotation` for placement. **6.27.3-dev**: dampeners brake tangent drift when not hovering, center-of-mass average prevents tipping, and grounded grids align to planet up. **7.5.0-dev** adds pressure-scaled aerodynamic drag and removes a radial maritime double-falloff, Unity-validated by Thomas. **7.6.0-dev** adds shared player/cockpit gravity-pull telemetry; **7.6.1-dev** refines it into fitted LCD field instruments; **7.7.0-dev** compacts the on-foot monitor while retaining the discrete surface-reference display, Unity-validated by Thomas. **7.8.0-dev** adds an honest coast-path solver and cockpit suborbital/orbital/escape diagnostics, Unity-validated by Thomas. **7.9.2-dev** makes occupied-cockpit dampeners hold full velocity at zero, samples flight altitude from rigidbody centre, adds vertical-speed telemetry, and preserves third-person Alt orbit until a double-Alt home reset. **7.13.0-dev** upgrades to REAL orbital mechanics: Keplerian propagation (elliptical orbits, vis-viva velocities, μ = g·r² per body), N-body inverse-square gravity (star + every body), reference-frame switching that conserves cosmic velocity, deep-space zero-g, and a deep-space flight-computer state. Full trajectory rendering and orbit-map controls remain future work. |
| Space stations | ❌ MISSING | No buildable orbital platforms. **7.13.0-dev** ships the foundation to reach them: real continuous flight between bodies (floating origin + frames) and deep-space minable asteroids. |
...
| Crafting / items / storage | 🛠️ WORKING ON | Core systems exist. **6.16.0-dev** adds global 900-unit matter stacks, player/container weight limits, matter-conversion storage GB usage, and inventory-weight UI. **7.10.0-dev** adds the Pirate Jack Pump Head relic, Tier-4 Pirate Oil Recovery research, expensive 90-second Jack Pump crafting, and infinite-node barrel extraction; **7.10.1-dev** keeps ordinary crude seeps finite and reserves infinite output for the marked Jack Pump nodes; **7.10.2-dev** allows finite crude collection on selected oil-rich worlds while retaining Planet_Pirate as the sole infinite Jack Pump source, Unity-validated by Thomas. **7.11.0-dev** unifies bucket/pump/tank/pipe liquid handling with native water/oil pool telemetry and persistence; Thomas confirmed native water rendering and reported terrain/cave/LOD follow-ups addressed in **7.11.1-dev** and **7.11.2-dev**. Full Unity validation is pending; deeper recipe chains and storage progression remain open. |
| Research / tech tree | 🟡 PARTIALLY COMPLETE | Core research exists; era expansion and final research UI validation remain open. |
| UI / UX | 🛠️ WORKING ON | Runtime crisp UI scaling, responsive machine panels, build-wheel fit, recipe validation tooling, drop-limit notifications, inventory-weight readout, and terminal search are active. **7.6.0–7.8.0-dev** gravity/orbital readouts are Unity-validated by Thomas. **7.8.1-dev** removes destructive Grid Battery UI refreshes that flashed mode controls. **7.9.0-dev** introduces a shared fitted LCD presentation across cockpit, suit vitals, held-item readout, and hotbar slots. **7.9.1-dev** rebuilds Inventory as a cargo-matrix display. **7.9.2-dev** keeps Armor / Jetpack / Life Support visible together, simplifies the Crafting command, removes the held-item slot prefix plus stale duplicate readouts, restores Grid Battery charging UI/interaction, and restyles Ship Control as the matching inventory-like LCD console; **7.11.14-dev** elevates all HUDs, machine panels, crafting screens, and terminal UIs with unified retro-futuristic LCD animations (animated scanline shimmer, CRT boot sequences, Tektronix bezel brackets, pulsing live dots, and tactile button micro-interactions); **7.11.15-dev** moves the world inspection HUD to the top right with LCD theme styling and multi-depth voxel/tree resolution; Unity validation is pending. |
| Top-right world inspection overlay | 🛠️ WORKING ON | Relocated to the top right (`right = 16`, `top = 18`) with fitted LCD instrument chassis and scanlines (**7.11.15-dev**). Features 3 surface-normal sample depths (`0.55m`, `0.25m`, `0.85m`) so aiming at dirt, grass, stone, or trees reliably displays material hardness, mining tier, and harvestability. Unity validation pending. |
...
| Water simulation | 🛠️ WORKING ON | **7.11.0-dev** removes the external ocean integration in favour of native curved spherical ocean rendering, voxel pools/lakes, radial wakes, finite/infinite pool pumping, pipe-aware buffers, and dense slow-sinking crude. Thomas confirmed native rendering works; **7.11.1-dev** through **7.11.8-dev** address authored planet scale, automatic solar system/asteroids, dry caves, liquid escape/mining, real ocean basins with seam-free ocean LOD, wrapped terrain, full-sphere LOD, HUD/LOD interaction feedback, and compile recovery. Full Unity validation is pending. |
| Weather system | 🛠️ WORKING ON | **9.19.0-dev** activates the previously-dormant weather stack: `CosmosBootstrap` now creates the `_Weather` controller; new additive `WeatherClimateProfile` on `BodySettings` drives per-planet precipitation/storms; `WeatherAllowed` auto-disables weather on airless bodies; `WeatherLighting` publishes a non-fighting sun-darkening + ambient tint consumed by `SunLightController`, owns rain/snow fog, and flashes synced lightning; thunder is unified (audio + flash on one event); Setup Step 58 authors themed climate profiles non-destructively. **9.20.0-dev** adds the cloud deck, visible rain/snow, and true 3D weather audio. **9.20.1-dev** hotfixes a case-sensitive typo (`TransformObjectToHclip` → `TransformObjectToHClip`) in the two new weather shaders that caused the magenta cloud dome and magenta/invisible particles. **9.21.0-dev** couples weather into wind: `WeatherManager` publishes a smoothed `WindMultiplier` that boosts wind-turbine output/degradation and the ambient `WindField` (grass/particles/audio) during storms — the wind half of the weather↔power coupling (solar already scaled with weather). **9.21.1-dev** fixes rain streaks (Stretch-mode `velocityScale` was 0) and darkens the sky dome during storms via a new `_VoxelWeatherDarken` global. **9.21.2-dev** replaces the unreliable Stretch-mode rain renderer with a Billboard + texture approach. **9.21.3-dev** rebuilds rain on the project's proven `SpaceDustRenderer` particle path: procedural billboard-UV streak/dot shapes (no texture), explicit material queue/ZWrite/cull, `allowOcclusionWhenDynamic = false`, plus a `[Weather] Rain:` console diagnostic. **9.21.4-dev** adds unconditional `[Weather] Manager heartbeat` / `[Weather] Rain heartbeat` logging so the console alone reveals whether weather is active on the current body (airless worlds have no weather by design) before any further rendering work. **9.21.5-dev** fixes the mixed-mode velocity-curve error (rain froze in the sky) and ducks weather audio under the UI. Unity validation pending by Thomas. |
| Camera / trajectory tools | 🛠️ WORKING ON | Basic camera foundations exist. **7.8.0-dev** adds a cockpit coast-path orbital solver/readout for tangential versus circular velocity and periapsis/apoapsis diagnostics, Unity-validated by Thomas; trajectory rendering, zoom-to-trajectory, and orbit-map controls remain open. |
...
| Armor crafting/upgrades | 🟡 PARTIALLY COMPLETE | **6.80.4-dev:** Thomas Unity-validated the dedicated Armor Station, anvil-style Armor Upgrade Station, module crafting, timed installation (30s at T1 through 150s at T5/Hazmat), per-piece state, equipment interaction, and additive save/load. **7.3.2-dev** adds tier-gated module capacity/grade rules (T1→1 slot/T1 through T6→6 slots/T5; Hazmat T5+) pending Unity validation. Step 48 authors all content non-destructively. Remaining: broader reactor/re-entry/heated-room hazard systems. |
...
| Factory persistence | ✅ COMPLETED | Conveyor/Chute item packets, Conveyor Splitter buffer+round-robin cursor+routing mode+per-output filters, Crusher/Assembler recipe+progress+enabled, Funnel buffer+mode, and all machine containers save and restore. Legacy saves compatible. |
