...
10. 🛠️ **WORKING ON — Finish vehicle power foundations validation** — 6.17.1-dev adds execution-order coordination, maritime-to-grid power sync, gas network proximity/corridor probes, pipe port snap full-cell seating, turbo snapping and physical-turbo engine effects, and drive shaft alignment. Thomas needs to validate the full maritime fuel→engine→shaft→generator→battery→propeller power chain end-to-end in Unity alongside the connectivity fixes.
...
13. 🛠️ **WORKING ON — Validate 6.18.0-dev variable ports, connectivity, exhaust orient & propellers in Unity** — rebuild maritime prefabs via **Tool → Voxel Engine → Voxel Engine Setup** (mesh v22→v23) for the new lofted propeller blades, then confirm: (a) aiming a liquid/gas pipe at an HFO/MGO engine face installs a color-coded fuel/coolant/oxygen port that sits outside the hull; (b) over-cap placement is refused with a red ghost + message; (c) a tank a few cells off a pipe run now connects and fuel/oxygen/coolant buffers fill; (d) the exhaust pipe's intake flange auto-orients down onto the engine collector; (e) propellers spin about the shaft and produce thrust; (f) variable ports + engine state persist across save/load.
