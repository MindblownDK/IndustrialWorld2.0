...
| Catalytic cracking & petrochemicals | 🟡 PARTIALLY COMPLETE | **9.40.0-dev** shipped the `CatalyticCracker` reactor unit — catalyst bed decay and reactor temperature both gate throughput through `CrackingEfficiency01` — with FCC / CCR / hydrocracking, synthetic resin, industrial lubricant, both catalyst syntheses, four items, `Block_CatalyticCracker` and the tier-6 chemistry gate (Step 71). **9.57.0-dev** closes the round's documented gap: bed percentage, reactor temperature, the batch in progress and all four tank contents are saved and restored. **9.58.0-dev** makes the panel readable while all of that changes: core temperature, catalyst bed, cracking efficiency, the four tank gauges, the batch bar, the status pill and the wattage are written in place every frame, so the readouts move without a rebuild touching the page scroll. Open: resin and lubricant have no consumers; no regenerator unit, no hydrogen plant, no grid variant. |
...
| Recipe registry refactor | 🟡 PARTIALLY COMPLETE | ScriptableObject crafting and machine recipes exist. **11.3.0-dev** consolidated the duplicate iron/copper ore assets onto one canonical item each (setup step 80), blanked the `ItemDefinition.itemId` default that made unauthored assets claim to be iron ore, and added an item-identity audit (step 79) plus retired-id save aliases. Pending Unity validation. Shaped/shapeless/smelting/machine unification and validation remain incomplete. |
