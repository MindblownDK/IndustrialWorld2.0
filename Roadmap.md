...
21. **Run setup wizard steps (non-destructive)**
    - Step 13 for maritime prefabs after drivetrain/port changes. **6.81.0-dev:** run it twice to refresh the v26 Watertight Shaft Housing mesh plus gearbox/generator/propeller/shaft port markers without overwriting ship balance or custom prefab work. Step 13 also creates the Mechanical Belt item/recipe and retires only new Encased Chain Drive discoverability; legacy ships remain intact.
