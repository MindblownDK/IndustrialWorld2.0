...
| Sky / atmosphere / space rendering | 🛠️ WORKING ON | **7.5.0-dev** starts a real upper-atmosphere-to-vacuum handoff: profile-driven air density, dark space camera transition, altitude-aware far clipping, and stable camera-relative celestial bodies without a duplicate home-planet proxy. Starfield/nebula polish and planet-specific sky art remain open pending Unity validation. |
| Gravity / orbits | 🛠️ WORKING ON | Grid gravity uses radial `GravityProvider` + `GetSurfaceRotation` for placement. **6.27.3-dev**: dampeners brake tangent drift when not hovering, center-of-mass average prevents tipping, and grounded grids align to planet up. **7.5.0-dev** adds pressure-scaled aerodynamic drag and removes a radial maritime double-falloff; true orbital trajectories remain future work and all new altitude behavior awaits Unity validation. |
