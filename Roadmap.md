...
- The deck level over a gap is the approach line plus a waterline freeboard (3.6 m fixed / 2.4 m
  drawbridge), with the rise ramped at 15% from both shores — only the excess above the line is
  ramped, so dry gullies and high banks keep the approach-following behaviour they always had.
  Deck levels are interpolated in absolute heights now, not lerped frame-relative offsets, and a
  bed probe with a 40 m reach replaced the 4 m road probe for piers and clearance, so piers find
  the riverbed and deep channels classify as bridges. The waterline itself is read through one
  helper that speaks both of the world's dialects of water — the voxels the fluid sim fills, and
  on the sphere the planet's ocean shell (which is not per-voxel water and was invisible to the
  voxel scan: over the open ocean the freeboard demand read as zero and the deck followed the
  shore line into the sea). Shallow margins are water too now, whatever their dry bed voxels say
  — the crossing no longer drapes road along the lakebed — and each gap span logs one diagnostic
  line at commit in editor/development builds, so the next report on a crossing is numbers.
- Fixed crossings carry two edge girders (a spine on wide decks) and capped masonry piers; the
  substructure bills **stone** (2/cell, wired by Step 73) alongside the deck's iron (6/cell).
  Drawbridges keep a clear channel — no piers under leaves. Also fixed: `AddCell` appended its
  per-cell flags before the seen-set check, so a shared corner shifted every later flag one cell.
- A station too far out of level to pave is now **cut and filled to one level at commit** (3 m
  each way, rough-ground price, buildings and >6 m cliffs still refuse) instead of being rejected
  with "level the strip first". Hand-placed blocks keep the old rules; the ghost shows green where
  the paver will grade.

### 9.44.3-dev — The Drawbridge Card Becomes Reachable
- The surface wheel's hover test split the screen in two, so the third card — DRAWBRIDGE ROAD —
  rendered but could never be hovered: 9.44.1's "reachable" was only half true, and the 9.44.2
  automation was unreachable through the surface that selects it. The defect is gone by
  construction, not by patch: nothing in the wheel hardcodes the segment count any more.
- The wheel is now `ConveyorShapeWheel`'s radial ring laid out as a triangle: three 120° slices
  (asphalt at the crown, pathway lower right, drawbridge lower left), pointer-event hover on the
  ring with the centre badge as a dead zone, click picks without closing, the BuildWheel key
  release commits and closes, and the badge carries the hovered surface's title and blurb.
- Runtime UI only: `BuildSystem` self-attaches the component, no setup step, no new assets, no
  saved fields.

### 9.44.2-dev — Drawbridges Answer the Water
- The four wiring items held back from 9.44.1 are in, and all of them are runtime behaviour on the
  span that already exists — no new assets, no setup step, no new saved fields. A drawbridge now
  **opens by itself for a hull in its channel**: one `OverlapBoxNonAlloc` under the deck every 0.5 s,
  classified along the channel axis, with the maritime verdict coming from
  `WaterProbeSystem.GetSubmergence` at the grid's centre of mass — a floating hull opens the deck, a
  car on the deck and a flyer overhead do not. Hysteresis (22 m approach, 35.2 m hold, 4 s grace)
  and a 2.6 m occupied-box above the deck mean it is never dropped on a hull or a stalled lorry, and
  the automation only ever opens on a fresh arrival and only ever shuts what it opened, so the hand
  on the lever always wins.
- **The swing is billed**: a `PowerConsumer` on the abutment cell demands 450 W while the deck moves
  and 0 W while it sits, self-healing onto a new abutment if the first cell is lifted. An unwired
  deck no longer swings — the refusal, the flashing beacons and a range-gated stall notice all say
  why. **Every swing sounds a procedural horn** (`Sfx.BridgeHorn`, 98 + 147 Hz, 140 m carry) and
  **red beacons on both approaches** run dark / flashing / steady with the deck's state.
- **The interact key works without the paver** — aim at a deck, get the standard `[E]` prompt. The
  same round fixed real 9.44.0/9.44.1 geometry defects the automation exposed: `DeckLength` counted
  every lane cell (3-lane leaves flew 3× too long, piers came in colonnades), the far leaf pointed
  away from its own crossing and tilted down, leaves were 1 m wide regardless of deck width, piers
...
> **Rule:** No personal names. Write every release note as if the team found and fixed the issue — "we saw an issue and resolved it", never "I did" and never a person's name.
...
| Unified grid placement | 🟡 PARTIALLY COMPLETE | Detail/Structural placement, shape variants, screen sources, and unified Item/Gas/Liquid pipe placement/networks are validated. **5.69.0-dev** adds additive movable-grid save/restore for Structural and Detail blocks, variants, settings, and attached pipes; Unity validation covered it in Unity. **7.5.0-dev** keeps ordinary blocks exactly lattice-centred when building from landing gear or any existing grid; Unity validation confirmed it. Remaining positional-indexing work keeps this broader area partially complete. |
...
| Building (static + tiered) | 🛠️ WORKING ON | 3.75 m spacing, scale, rotation, and player-away Doors are Unity-validated. Unity validation also covered the 6.4.6-dev static placed-block tangent-anchor snap for cleaner same-level planetary placement. Tiered overlap validation is synchronized with the intended 0.45 half-extents. Size-V5 closes Foundation deck seams and adds upward/downward Stair anchors at Foundation/Floor edges and Doorway thresholds; final validation is pending. |
...
| Grid screens / displays | ✅ COMPLETED | All sizes, live text+power states, right-click+terminal config, custom text+custom colors+border+font, visual bar charts, multi-source, live camera feeds, power gain/loss/net mode, persistence, and camera block are validated in Unity. (5.51.3-dev) |
...
| Grid shape variant wheel | ✅ COMPLETED | Unity validation covered all six structural variants, textured collision meshes, accurate ghosts, and the corrected radial-wheel slice alignment in 5.63.2-dev. Step 18 remains the non-destructive authoring path. |
...
| Water simulation | 🛠️ WORKING ON | **7.11.0-dev** removes the external ocean integration in favour of native curved spherical ocean rendering, voxel pools/lakes, radial wakes, finite/infinite pool pumping, pipe-aware buffers, and dense slow-sinking crude. Unity testing confirmed native rendering works; **7.11.1-dev** through **7.11.8-dev** address authored planet scale, automatic solar system/asteroids, dry caves, liquid escape/mining, real ocean basins with seam-free ocean LOD, wrapped terrain, full-sphere LOD, HUD/LOD interaction feedback, and compile recovery. Full Unity validation is pending. |
| Weather system | 🛠️ WORKING ON | **9.19.0-dev** activates the previously-dormant weather stack: `CosmosBootstrap` now creates the `_Weather` controller; new additive `WeatherClimateProfile` on `BodySettings` drives per-planet precipitation/storms; `WeatherAllowed` auto-disables weather on airless bodies; `WeatherLighting` publishes a non-fighting sun-darkening + ambient tint consumed by `SunLightController`, owns rain/snow fog, and flashes synced lightning; thunder is unified (audio + flash on one event); Setup Step 58 authors themed climate profiles non-destructively. **9.21.x-dev** made rain actually fall (velocity-curve fix) and kept streaks velocity-aligned. **9.22.0-dev** replaces the cloud sky: the old 320 m spherical caps that ended at ~31° elevation (the "round circle" overhead) are gone, replaced by horizon-fitted low/high cloud decks placed in view-elevation space with their rim below the eye line, planar world-anchored UVs (no zenith pinch, real parallax when walking), coverage-threshold shaping instead of opacity fading, billow-noise masses eroded by a second scrolling octave, gradient-normal lighting with dark rain-bellies and lit crowns, forward-scattering thin edges, vertex-puffed undersides and a horizon blend into the live fog colour. **9.23.0-dev** goes the whole way: the camera-following deck (which smeared stretched curtains at its far clamp) is replaced by REAL CLOUD SHELLS wrapped around every atmospheric body — a shared icosphere scaled to surface radius + cloud altitude, parented to the body so clouds never follow the player to space, density sampled from a tileable 3D volume in body-local space (no poles, no seams, no UV pinch), a 4-step view-ray march for volumetric parallax from inside and outside, drifting weather cells that make coverage regional and dynamic, storm-driven near-black rain-bellies with lit crowns, per-planet climate skies for worlds seen from orbit, and rain restored to the proven velocity-aligned billboard path. Developer hotkeys Ctrl+Alt+W / Ctrl+Alt+R force weather states for testing. **9.23.1-dev** shortens rain streaks to 0.45–0.80 m (long quads went edge-on when looking along the fall axis and smeared into radial warp lines) and splits weather into planet-wide `Intensity` versus player-local `SurfaceProximity`/`LocalIntensity`, so rain, splashes, weather audio, storm fog and sun darkening fade out exactly at the visible cloud deck and never follow the player into space. **9.23.2-dev** fixes sideways rain at the source: the fall vector is now written in world space from the body's radial down every frame instead of the emitter's local -Y, which tipped whenever the camera looked near-vertically and `OrthoNormalize` degenerated; blizzard gusts moved onto the surface tangent and the weather frame gained a degenerate-basis guard. Unity validation pending. **9.23.3-dev** removes the last direction bug: Unity's Velocity alignment rotates the quad's X axis onto the velocity while the particle shader draws its streak along V, so correctly falling drops were drawn as horizontal slashes; rain is now a plain camera-facing billboard whose streak is oriented in-shader from the projected global `_WeatherFallDir`, foreshortening naturally when viewed along the fall. **9.24.0-dev** makes weather physical: wind applies a real mass-correct aerodynamic force to grids that are not locked by landing gear or a docking port (sail area from the block budget, applied above the centre of mass so hulls heel and weathervane, tangent-projected so gusts never lift a grid, capped at a fraction of grid weight), and a new `WeatherSeaState` publishes a global sea state with realistic build/decay inertia that raises wave height, speed, chop, wind-aligned swell direction and whitecaps across coastal water, pools and the distant ocean LOD, with floating grids riding the wind-driven surface current. **9.25.0-dev** adds planetary seasons (4-season astronomical orbital calendar with annual temperature, solar, and wind oscillations), snowfall and howling blizzards with procedural feathered snowflakes, ground snow settling across surfaces/blocks in winter, the Grid Season Monitor & Grand Planetary Observatory ground blocks + Screen Data Object telemetry provider for screens, on-foot and cockpit HUD ambient temperature telemetry, and weather particle performance optimizations (zero-allocation curve updates, early-Z alpha clipping, zero-GC audio scanning). Unity validation pending. |
...
> stepping). The road
> **network** object, routing that prefers paved routes and lamps remain open items below. Full
...
22. ~~Author **asphalt roads** as terrain-placed surface blocks (straight, bend, junction, crossing,
    ...
    runtime behaviour with no new setup step of their own. `RoadRun`
    ...
    and stamina bonuses, rolling resistance, roadside lamps/furniture, a drawbridge barrier arm and a
    per-span automation switch.)*
