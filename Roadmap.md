...
- **6.24.0-dev:** Added Space Helmet and Oxygen Tank equipment items/recipes, Life Support equipment slots in Inventory, save/load for helmet/tank slots, and underwater oxygen reserve/drain reduction when helmet+tank are equipped. Remaining work: active oxygen-rich environment checks, offline death rules, cryobed power/oxygen validation, vacuum/room-pressure integration, and final Unity validation.
...
13. 🛠️ **WORKING ON — Validate 6.19.0-dev round-2 fixes in Unity** — rebuild maritime prefabs via **Tool → Voxel Engine → Voxel Engine Setup** (mesh v23→v24) and confirm: (a) propellers show real blades; (b) variable port collars sit flush on the hull and are slim around a gas pipe; (c) pipe hubs snap to the fine Detail lattice; (d) the Crude engine takes Oxygen + Item variable ports and has no static fuel/coolant/oxygen/item ports (exhaust + shaft remain); (e) a second fuel run is refused with "Fuel input already connected (max 1)"; (f) a tank ~0.5 m off a run (face + diagonal) connects and the engine runs; (g) 30+ nearby pipes no longer lag and only update arms on placement/removal; (h) variable ports persist across save/load.
