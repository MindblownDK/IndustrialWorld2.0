...
- **6.22.3-dev:** Jetpack Bay no longer uses a heavy boxed frame, the ONLINE/EMPTY pill sits beside the title on the left, drag/drop slot changes refresh immediately, and Hydrogen Boost Pack sprint boost has a short subtle spool-up. **6.72.0-dev:** Fuel/power accounting shipped (capacity, flight/boost drain, Hydrogen Cell + Charged Cell auto-refuel, Jetpack Bay fuel bar, empty-pack flight cut). Remaining: armor-station upgrade integration and final Unity validation.
