...
| Water simulation | 🛠️ WORKING ON | **7.11.0-dev** removes the external ocean integration in favour of native curved spherical ocean rendering, voxel pools/lakes, radial wakes, finite/infinite pool pumping, pipe-aware buffers, and dense slow-sinking crude. Thomas confirmed native rendering works; **7.11.1-dev** through **7.11.6-dev** address authored planet scale, dry caves, liquid escape/mining, real ocean basins with seam-free ocean LOD, wrapped terrain, full-sphere LOD, and HUD/LOD interaction feedback. Full Unity validation is pending. |
