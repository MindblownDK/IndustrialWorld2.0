...
## [3.23.2] — Flat Ocean Mesh + No Crest Generation

**Type:** PATCH — Save-compatible fix for two visible problems in the
[3.23.1] screenshot: ugly voxel stair-step water surface and diagonal
chunk-boundary gaps. Also stops the wizard from creating Crest at all.

### Fixed
- **"Stair-step" voxel look on the ocean.** Every water column had its
  own per-voxel `finalH` (mesh height in chunk-local units), so
  neighbouring columns at slightly different water levels produced tiny
  vertical steps across the whole ocean — like the "circuit board"
  pattern in the screenshot. Fixed by flattening any Water cell within
  4 voxels of sea level to *exactly* sea level. The `VoxelWaterURP`
  shader then adds Gerstner-wave displacement on top, so you get a
  smooth animated ocean plane instead of stepped voxel geometry.
- **Diagonal chunk-boundary gaps (bare terrain stripes).** When
  `AccumulateCornerCell` looked up a corner outside the current chunk
  it just returned nothing → the corner averaged only the in-chunk
  cells, so two neighbouring chunks produced slightly different corner
  heights → visible seam. Fixed by having out-of-bounds lookups return
  sea level directly (both chunks agree on the same corner height →
  seamless).
- **Wizard was creating a `Crest Ocean` GameObject even when none was
  needed.** Now the wizard *destroys* any Crest GO it finds and does
  NOT create a new one. Runtime `CrestVoxelWaterBinder` also actively
  destroys any Crest that shows up (was previously just disabling).

### Added
- `WaterMeshBuilder.FlattenOceanToSeaLevel` (bool, default `true`) —
  master toggle for the flat-plane ocean.
- `WaterMeshBuilder.FlattenBandVoxels` (float, default `4.0`) —
  vertical band around sea level where flattening applies. Cells
  outside this band (mountain lakes, elevated rivers) keep their
  per-column height.
- Planet-world flattening: solves the axial coordinate that puts each
  cell on the sea sphere so planet oceans are smooth too.

### Changed
- `CrestVoxelWaterBinder.HideCrestOceanTiles` — now destroys rather
  than disables. If the user reactivates a Crest GO, it gets nuked
  again on the next binder tick.
- `CrestWaterSetupUtility.EnsureCrestOceanRendererInScene(...)` — no
  longer *ensures*; it now *removes*. Kept the same method name for
  minimal call-site churn.
- `CornerData` / `AccumulateCornerCell` now take the source `Chunk`
  and the dominant axis so cross-chunk seam stitching can compute the
  right world position.
- `GameVersion` bumped to `3.23.2-dev`.

### Manual Unity steps
1. Pull, let Unity recompile.
2. Menu: **Tools → Voxel Engine → Configure Crest Water Integration**.
   Dialog now reads *"Flat Ocean Mesh + No Crest configured"* and if
   any Crest Ocean GO existed in the scene, reports it was destroyed.
3. In the Hierarchy: **no `Crest Ocean` GameObject should exist.**
4. Press Play. Expected:
   - Ocean is a **smooth plane** at sea level with animated waves —
     no more stepped voxel look.
   - **No diagonal terrain stripes** between chunks.
   - Mountain lakes still show at their local elevation.
5. If any water surface still looks stepped: check
   `Assets/Resources/CrestOcean_VoxelBridge.mat` — you can tune
   `_DeepWaveAmplitude` down or up for calmer / bigger waves. The
   flatness is now guaranteed by the mesher regardless.
6. If a `Crest Ocean` GO reappears at runtime, tell me which prefab
   or Awake path is spawning it — I'll add it to the nuke list.
...
## [3.23.1] — Actually Kill Ocean Plane + Premium Water Look *(superseded by 3.23.2)*
