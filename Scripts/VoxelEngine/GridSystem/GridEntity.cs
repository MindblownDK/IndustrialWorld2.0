...
//       Each block snaps to cell_size * gridPos offset from center of mass.
//
...
namespace VoxelEngine.GridSystem
{
    ...
    public class GridEntity : MonoBehaviour
    {
        ...
        // ── Thrust ─────────────────────────────────────────────────
        public Vector3 ThrustInput { get; set; } // set by Cockpit: normalized (-1..1) per axis
        ...
        public void RemoveBlock(Vector3Int gridPos)
        {
            ...
            // If no blocks remain, destroy the grid entity.
            if (_blocks.Count == 0)
                Destroy(gameObject);
        }
        ...
        /// <summary>Convert a world position to the nearest grid cell position.</summary>
        ...
        private void UpdateThrust()
        {
            ...
            foreach (var kv in _blocks)
            {
                ...
                {
                    // Thruster contributes force along its local forward direction.
                    ...
                    // Torque from off-center thrusters.
            ...
            // Rotation from cockpit input.
            Vector3 rotInput = new Vector3(RotationPitch, RotationYaw, RotationRoll);
            float rotPower = 5000f * _rb.mass; // scale with mass
            _rb.AddTorque(transform.TransformDirection(rotInput) * rotPower * Time.fixedDeltaTime,
                ForceMode.Force);
        }
        ...
        private void UpdateDampeners()
        {
            ...
            if (ThrustInput.sqrMagnitude > 0.01f) return; // player is thrusting, don't dampen

            // Slow down linear velocity gradually.
        ...
        /// <summary>Create a new empty grid entity at the given position.</summary>
