...
namespace VoxelEngine.Transport
{
    ...
    public class Quarry : MonoBehaviour
    {
        ...
        // Model bounds - computed after first frame via coroutine
        private float _mbY = -999f; // model bottom Y
        private float _mtY = -999f; // model top Y
        ...
        private GameObject _gh, _ta, _dr, _bm;
        ...
        void Start()
        {
            _w = VoxelWorld.Instance;
            _mr = _w?.materialRegistry;
            _pc = GetComponent<PowerConsumer>();
            if (_w == null) { enabled = false; return; }
            _org = _w.WorldToVoxel(transform.position);

            // Default fallback: model is 2.4m cube centred on position
            ...
            // Defer bounds detection one frame (colliders/renderers may not be ready in Start)
        ...
        IEnumerator CaptureModelBounds()
        {
            ...
            // Try collider first, then renderer
            ...
            Debug.Log($"[Quarry] Bounds captured: bottom={_mbY:F2} top={_mtY:F2} (fallback was ±1.2 from {transform.position.y:F2})");
        }
        ...
        void Update()
        {
            ...
            if (Phase == QuarryPhase.Idle && _gh == null) CGh();
            else if (Phase != QuarryPhase.Idle && _gh) DGh();
            ...
            if (EffSize != _ls) { _ls = EffSize; RecomputeArea(); CalculateMaxDepth();
                ...
                else if (Phase == QuarryPhase.Idle) { DGh(); CGh(); } }
        ...
        void DA() { DGh(); DTp(); foreach (var fb in _fs) if (fb) Destroy(fb); _fs.Clear(); _fp.Clear(); if (_dr) { Destroy(_dr); _dr = null; } if (_bm) { Destroy(_bm); _bm = null; } }
        void ETP() { Phase = QuarryPhase.TapeFrame; _tt = 0f; DGh(); CTp(); }
        ...
        void GL(Vector3 a, Vector3 b, Color c) { var go = new GameObject("L"); go.transform.SetParent(_gh.transform, false); var lr = go.AddComponent<LineRenderer>(); lr.positionCount = 2; lr.SetPositions(new[]{a,b}); lr.startWidth = lr.endWidth = 0.06f; lr.startColor = lr.endColor = c; lr.useWorldSpace = true; var sh = Shader.Find("Universal Render Pipeline/Particles/Unlit") ?? Shader.Find("Sprites/Default"); lr.material = new Material(sh) { color = c }; lr.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off; }
        void DGh() { if (_gh) { Destroy(_gh); _gh = null; } }
        ...
        void OnDestroy() { DGh(); DTp(); HPP(); foreach(var fb in _fs) if(fb)Destroy(fb); _fs.Clear(); if(_dr)Destroy(_dr); if(_bm)Destroy(_bm); }
