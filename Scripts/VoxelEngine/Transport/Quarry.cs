...
namespace VoxelEngine.Transport
{
    ...
    public class Quarry : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (EffSize != _ls)
            {
                _ls = EffSize; RecomputeArea(); CalculateMaxDepth();
        ...
        public static void ShowPlacementPreview(Vector3 wp, Quaternion rot, int sz, float fo)
        {
            ...
            // For placement ghost: top = surface + 0.5, bottom = block center - modelHalfHeight
            float yTop = vox.y + 0.5f;
            float yBot = wp.y - 1.2f;
        ...
        public static void HPP() { if (_pp) { Object.Destroy(_pp); _pp = null; } }
        ...
        // ═══ GHOST (placed quarry in idle) ═══════════════
        void CGh()
        {
            if (_gh) Destroy(_gh); _gh = new("QG");
            float yTop = transform.position.y + 1.2f;  // model is 2.4m tall, half=1.2
            float yBot = transform.position.y - 1.2f;
        ...
        void PF()
        {
            ...
            float yBot = transform.position.y - 1.2f;  // model bottom (= quarry block bottom)
