...
namespace VoxelEngine.Transport
{
    ...
    public class Quarry : MonoBehaviour
    {
        ...
        {
            InstalledRangeLevel = InstalledSpeedLevel = InstalledEfficiencyLevel = 0;
            if (upgradeC == null) return;
            for (int i = 0; i < upgradeC.Size; i++)
            {
                var s = upgradeC.GetSlot(i);
                if (s.IsEmpty || !(s.item is QuarryUpgradeItem u)) continue;
                int add = u.level * s.count;
                switch (u.upgradeKind) { case QuarryUpgradeKind.Range: InstalledRangeLevel = Mathf.Min(MaxRangeLevel, add); break; case QuarryUpgradeKind.Speed: InstalledSpeedLevel = Mathf.Min(MaxSpeedLevel, add); break; case QuarryUpgradeKind.Efficiency: InstalledEfficiencyLevel = Mathf.Min(MaxEfficiencyLevel, add); break; }
            }
        }
        ...
        private void CreateGhostPreview()
        {
            ...
            Vector3 v2 = new(AreaMax.x + 1, yTop, AreaMax.z + 1), v3 = new(AreaMin.x, yTop, AreaMax.z + 1);
            GL(v0, v1, gc); GL(v1, v2, gc); GL(v2, v3, gc); GL(v3, v0, gc);
            GL(v0, new(v0.x, yBot, v0.z), gc); GL(v1, new(v1.x, yBot, v1.z), gc);
            GL(v2, new(v2.x, yBot, v2.z), gc); GL(v3, new(v3.x, yBot, v3.z), gc);
            Vector3 b0 = new(AreaMin.x, yBot, AreaMin.z), b1 = new(AreaMax.x + 1, yBot, AreaMin.z);
            Vector3 b2 = new(AreaMax.x + 1, yBot, AreaMax.z + 1), b3 = new(AreaMin.x, yBot, AreaMax.z + 1);
