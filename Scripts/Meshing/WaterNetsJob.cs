...
namespace VoxelEngine.Meshing
{
    ...
    public unsafe struct WaterNetsJob : IJob
    {
        ...
        public void Execute()
        {
            ...
            for (int cx = 0; cx < S + 1; cx++)
            {
                ...
                int vd000 = IsWaterSolid(voxels[i000]) ? 1 : -1;
                int vd100 = IsWaterSolid(voxels[i100]) ? 1 : -1;
                int vd010 = IsWaterSolid(voxels[i010]) ? 1 : -1;
                int vd110 = IsWaterSolid(voxels[i110]) ? 1 : -1;
                int vd001 = IsWaterSolid(voxels[i001]) ? 1 : -1;
                int vd101 = IsWaterSolid(voxels[i101]) ? 1 : -1;
                int vd011 = IsWaterSolid(voxels[i011]) ? 1 : -1;
                int vd111 = IsWaterSolid(voxels[i111]) ? 1 : -1;

                // Only process cells that contain at least one actual fluid voxel,
                // so we don't accidentally mesh solid terrain vs air!
                bool hasFluid = IsFluidMat(voxels[i000].material) || IsFluidMat(voxels[i100].material) || 
                                IsFluidMat(voxels[i010].material) || IsFluidMat(voxels[i110].material) || 
                                IsFluidMat(voxels[i001].material) || IsFluidMat(voxels[i101].material) || 
                                IsFluidMat(voxels[i011].material) || IsFluidMat(voxels[i111].material);
                
                if (!hasFluid) continue;

                // Mask uses VIRTUAL densities — fluid and terrain are solid
                int mask = 0;
                if (vd000 > 0) mask |= 1;
                if (vd100 > 0) mask |= 2;
                if (vd010 > 0) mask |= 4;
            ...
            {
                int idx = Idx(x, y, z);
                bool s0 = IsWaterSolid(voxels[idx]);

                // +X
                int idxX = Idx(x + 1, y, z);
                bool sX = IsWaterSolid(voxels[idxX]);
                if (s0 != sX)
                {
                ...
                bool sY = IsWaterSolid(voxels[idxY]);
                if (s0 != sY)
                {
                ...
                bool sZ = IsWaterSolid(voxels[idxZ]);
                if (s0 != sZ)
                {
        ...
        /// </summary>
        private bool IsWaterSolid(Voxel v)
        {
            return IsFluidMat(v.material);
        }
