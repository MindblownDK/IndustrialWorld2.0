...
namespace VoxelEngine.FX
{
    public class WorldAudioBootstrap : MonoBehaviour
    {
        ...
        private void SweepMachines()
        {
            ...
            foreach (var m in FindObjectsByType<DistillationPlant>(FindObjectsInactive.Exclude))
                Attach(m, Sfx.EngineRumble, () => (m.IsOnline && m.Current != null) ? 1f : 0f, vol: 0.7f, dist: 28f, basePitch: 0.55f, pitchSpread: 0.1f);
