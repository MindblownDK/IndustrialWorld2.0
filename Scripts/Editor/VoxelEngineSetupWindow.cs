...
namespace VoxelEngine.EditorTools
{
    ...
    {
        private static VoxelEngine.Crafting.RecipeDefinition AddRecipe(VoxelEngine.Crafting.RecipeRegistry registry, string folder, string assetName, string display, VoxelEngine.Items.ItemDefinition output, int outputCount, VoxelEngine.Crafting.StationTier station, bool unlockedByDefault, params (VoxelEngine.Items.ItemDefinition item, int n)[] inputs)
        {
        ...
        private static VoxelEngine.Crafting.RecipeDefinition AddRecipe(string assetName, string display, VoxelEngine.Items.ItemDefinition output, int outputCount, VoxelEngine.Crafting.StationTier station, bool unlockedByDefault, params (VoxelEngine.Items.ItemDefinition item, int n)[] inputs)
        {
            var reg = UnityEditor.AssetDatabase.LoadAssetAtPath<VoxelEngine.Crafting.RecipeRegistry>("Assets/VoxelEngineAssets/RecipeRegistry.asset");
            return AddRecipe(reg, "Assets/VoxelEngineAssets/Recipes", assetName, display, output, outputCount, station, unlockedByDefault, inputs);
        }

        private static VoxelEngine.Crafting.RecipeDefinition AddRecipe(string folder, string assetName, string display, VoxelEngine.Items.ItemDefinition output, int outputCount, VoxelEngine.Crafting.StationTier station, bool unlockedByDefault, params (VoxelEngine.Items.ItemDefinition item, int n)[] inputs)
        {
            var reg = UnityEditor.AssetDatabase.LoadAssetAtPath<VoxelEngine.Crafting.RecipeRegistry>("Assets/VoxelEngineAssets/RecipeRegistry.asset");
            return AddRecipe(reg, folder, assetName, display, output, outputCount, station, unlockedByDefault, inputs);
        }
        ...
        private void CreateAllAssets()
        {
            ...
            // ── Celestial world surface materials (Phase 2) — mine → Stone
            Make(MaterialId.MartianDust,    "Martian Dust",    new Color(0.62f,0.34f,0.20f), 0.5f, itemMap[MaterialId.Stone]);
            Make(MaterialId.VenusAsh,       "Venus Ash",       new Color(0.72f,0.62f,0.26f), 0.5f, itemMap[MaterialId.Stone]);
            Make(MaterialId.AcidBog,        "Acid Bog",        new Color(0.40f,0.52f,0.24f), 0.5f, itemMap[MaterialId.Stone]);
            Make(MaterialId.VolcanicBasalt, "Volcanic Basalt", new Color(0.14f,0.12f,0.13f), 1.2f, itemMap[MaterialId.Stone]);
            Make(MaterialId.CrystalGeode,   "Crystal Geode",   new Color(0.56f,0.48f,0.84f), 0.9f, itemMap[MaterialId.Stone]);
        ...
        private void BuildMainMenuScene()
        {
            ...
            {
                foreach (var sceneDocument in sceneRoot.GetComponentsInChildren<UnityEngine.UIElements.UIDocument>(true))
                {
                    sceneDocument.panelSettings = panelSettings;
                    EditorUtility.SetDirty(sceneDocument);
                }
            }
            UnityEditor.SceneManagement.EditorSceneManager.SaveScene(currentScene, gameScenePath);
            Debug.Log("[Wizard] Step 3 assigned MenuPanelSettings to every UIDocument in the Game scene.");
        ...
        // ============================================================
        private void BuildBaseCraftingContent()
        {
        ...
        private void BuildTieredContent()
        {
            ...
            void BuildPremiumFoundation(GameObject root, Material material)
            {
                ...
                AddBox(root, material, ScaleConstruction(new Vector3(0f, 0.35f, 1.43f)), ScaleConstruction(new Vector3(3.02f, 0.24f, 0.16f))).name = "Generated_FoundationBeamFront";
                AddBox(root, material, ScaleConstruction(new Vector3(0f, 0.35f, -1.43f)), ScaleConstruction(new Vector3(3.02f, 0.24f, 0.16f))).name = "Generated_FoundationBeamBack";
                AddBox(root, material, ScaleConstruction(new Vector3(1.43f, 0.35f, 0f)), ScaleConstruction(new Vector3(0.16f, 0.24f, 2.72f))).name = "Generated_FoundationBeamRight";
                AddBox(root, material, ScaleConstruction(new Vector3(-1.43f, 0.35f, 0f)), ScaleConstruction(new Vector3(0.16f, 0.24f, 2.72f))).name = "Generated_FoundationBeamLeft";

                foreach (var x in new[] { -1.25f, 1.25f })
                foreach (var z in new[] { -1.25f, 1.25f })
                {
                    var leg = AddBox(root, material,
                        ScaleConstruction(new Vector3(x, -0.02f, z)),
                        ScaleConstruction(new Vector3(0.24f, 0.86f, 0.24f)));
                    leg.name = $"Generated_FoundationLeg_{(x < 0 ? "L" : "R")}_{(z < 0 ? "B" : "F")}";
                }

                var braceA = AddBox(root, material,
                    ScaleConstruction(new Vector3(-0.68f, 0.12f, 1.42f)),
                    ScaleConstruction(new Vector3(0.11f, 1.50f, 0.10f)));
                braceA.name = "Generated_FoundationBraceA";
                braceA.transform.localRotation = Quaternion.Euler(0f, 0f, -55f);
                var braceB = AddBox(root, material,
                    ScaleConstruction(new Vector3(0.68f, 0.12f, 1.42f)),
                    ScaleConstruction(new Vector3(0.11f, 1.50f, 0.10f)));
            ...
            void BuildPremiumWall(GameObject root, Material material)
            {
                AddBox(root, material, ScaleConstruction(new Vector3(0f, 1.5f, 0f)), ScaleConstruction(new Vector3(3.0f, 3.0f, 0.16f))).name = "Generated_WallPanel";
                AddBox(root, material, ScaleConstruction(new Vector3(-1.42f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.16f, 3.0f, 0.30f))).name = "Generated_WallPostLeft";
                AddBox(root, material, ScaleConstruction(new Vector3(1.42f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.16f, 3.0f, 0.30f))).name = "Generated_WallPostRight";
                AddBox(root, material, ScaleConstruction(new Vector3(0f, 0.08f, 0f)), ScaleConstruction(new Vector3(2.84f, 0.16f, 0.28f))).name = "Generated_WallBeamBottom";
                AddBox(root, material, ScaleConstruction(new Vector3(0f, 2.92f, 0f)), ScaleConstruction(new Vector3(2.84f, 0.16f, 0.28f))).name = "Generated_WallBeamTop";
                AddBox(root, material, ScaleConstruction(new Vector3(0f, 1.5f, -0.11f)), ScaleConstruction(new Vector3(0.10f, 2.72f, 0.05f))).name = "Generated_WallCenterSeam";
            }
            ...
            void EnsureTieredDoor(GameObject root, Material material)
            {
                ...
                if (hinge == null)
                {
                    ...
                    hingeObject.transform.localPosition = ScaleConstruction(new Vector3(-0.88f, 0f, 0f));
                    hinge = hingeObject.transform;

                    var panel = GameObject.CreatePrimitive(PrimitiveType.Cube);
                    panel.name = "Generated_DoorPanel";
                    panel.transform.SetParent(hinge, false);
                    panel.transform.localPosition = ScaleConstruction(new Vector3(0.87f, 1.32f, 0f));
                    panel.transform.localScale = ScaleConstruction(new Vector3(1.74f, 2.60f, 0.12f));
                    panel.GetComponent<Renderer>().sharedMaterial = material;

                    AddBox(hingeObject, material, ScaleConstruction(new Vector3(0.87f, 2.48f, -0.08f)), ScaleConstruction(new Vector3(1.58f, 0.10f, 0.05f))).name = "Generated_DoorRailTop";
                    AddBox(hingeObject, material, ScaleConstruction(new Vector3(0.87f, 0.14f, -0.08f)), ScaleConstruction(new Vector3(1.58f, 0.10f, 0.05f))).name = "Generated_DoorRailBottom";
                    AddBox(hingeObject, material, ScaleConstruction(new Vector3(0.10f, 1.31f, -0.08f)), ScaleConstruction(new Vector3(0.10f, 2.24f, 0.05f))).name = "Generated_DoorRailHinge";
                    AddBox(hingeObject, material, ScaleConstruction(new Vector3(1.64f, 1.31f, -0.08f)), ScaleConstruction(new Vector3(0.10f, 2.24f, 0.05f))).name = "Generated_DoorRailLatch";
                    AddBox(hingeObject, material, ScaleConstruction(new Vector3(0.87f, 1.31f, -0.08f)), ScaleConstruction(new Vector3(1.48f, 0.09f, 0.05f))).name = "Generated_DoorRailCenter";
                    ...
                    // These values are panel-local; the panel's 25% scale carries
                    // the handle proportionally without applying the factor twice.
            ...
            // Every generated visual is 25% larger than Size-V3. Socket positions
            // describe the incoming piece root, not merely the host mesh edge.

            // FOUNDATION — raised 3.75 m deck with exact center-to-center neighbors.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Foundation, "Foundation",
                ...
                (root) => {
                    ...
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, new Vector3(0f, 0.75f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Foundation);

                    // An incoming Foundation root must be a complete module away.
                    // The previous half-module positions placed half of the new deck
                    // inside the host Foundation.
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.North, new Vector3(0f, 0f, constructionModuleSize), VoxelEngine.Building.Tiered.BuildFamily.Foundation);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.South, new Vector3(0f, 0f, -constructionModuleSize), VoxelEngine.Building.Tiered.BuildFamily.Foundation);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.East, new Vector3(constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Foundation);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.West, new Vector3(-constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Foundation);

                    // Wall-like roots sit on the true upper perimeter edge.
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.TopNorth, new Vector3(0f, 0.75f, halfModule), VoxelEngine.Building.Tiered.BuildFamily.Foundation, 0f);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.TopSouth, new Vector3(0f, 0.75f, -halfModule), VoxelEngine.Building.Tiered.BuildFamily.Foundation, 180f);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.TopEast, new Vector3(halfModule, 0.75f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Foundation, 90f);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.TopWest, new Vector3(-halfModule, 0.75f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Foundation, -90f);
                },
                Cost((woodLog, 4)),
                Cost((stone, 8)),
                Cost((ironIngot, 4)),
            ...
            // WALL — 3.75 x 3.75 m detailed panel.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Wall, "Wall",
                (root, mat) => { BuildPremiumWall(root, mat); },
                (root) => {
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, new Vector3(0f, constructionModuleSize, 0f), VoxelEngine.Building.Tiered.BuildFamily.Wall);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.East, new Vector3(constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Wall);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.West, new Vector3(-constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Wall);
                },
            ...
            // DOORWAY — 25%-larger structural frame with a separate Door socket.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Doorway, "Doorway",
                (root, mat) => {
                    AddBox(root, mat, ScaleConstruction(new Vector3(-1.2f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.60f, 3.0f, 0.24f)));
                    AddBox(root, mat, ScaleConstruction(new Vector3(1.2f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.60f, 3.0f, 0.24f)));
                    AddBox(root, mat, ScaleConstruction(new Vector3(0f, 2.82f, 0f)), ScaleConstruction(new Vector3(1.80f, 0.36f, 0.24f)));
                },
                (root) => {
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, new Vector3(0f, constructionModuleSize, 0f), VoxelEngine.Building.Tiered.BuildFamily.Doorway);
            ...
            // WINDOW — enlarged frame with a proportional mid-height opening.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Window, "Window",
                (root, mat) => {
                    AddBox(root, mat, ScaleConstruction(new Vector3(-1.24f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.52f, 3.0f, 0.24f)));
                    AddBox(root, mat, ScaleConstruction(new Vector3(1.24f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.52f, 3.0f, 0.24f)));
                    AddBox(root, mat, ScaleConstruction(new Vector3(0f, 2.66f, 0f)), ScaleConstruction(new Vector3(1.96f, 0.68f, 0.24f)));
                    AddBox(root, mat, ScaleConstruction(new Vector3(0f, 0.34f, 0f)), ScaleConstruction(new Vector3(1.96f, 0.68f, 0.24f)));
                },
                (root) => {
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, new Vector3(0f, constructionModuleSize, 0f), VoxelEngine.Building.Tiered.BuildFamily.Window);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.East, new Vector3(constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Window);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.West, new Vector3(-constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Window);
                },
            ...
            // FLOOR — 3.75 m square slab.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Floor, "Floor",
                (root, mat) => { AddBox(root, mat, ScaleConstruction(new Vector3(0f, 0.10f, 0f)), ScaleConstruction(new Vector3(3.0f, 0.20f, 3.0f))); },
                (root) => {
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, ScaleConstruction(new Vector3(0f, 0.20f, 0f)), VoxelEngine.Building.Tiered.BuildFamily.Floor);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.North, new Vector3(0f, 0f, constructionModuleSize), VoxelEngine.Building.Tiered.BuildFamily.Floor);
            ...
            // STAIRS — twelve proportional steps across one complete module.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Stairs, "Stairs",
                (root, mat) => {
                    for (int i = 0; i < 12; i++)
                    {
                        AddBox(root, mat,
                            ScaleConstruction(new Vector3(0f, 0.125f + i * 0.25f, -1.375f + i * 0.25f)),
                            ScaleConstruction(new Vector3(3.0f, 0.25f, 0.25f)));
                    }
                },
            ...
            // ROOF — enlarged sloped slab.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Roof, "Roof",
                (root, mat) => {
                    var go = AddBox(root, mat, ScaleConstruction(new Vector3(0f, 0.65f, 0f)), ScaleConstruction(new Vector3(3.0f, 0.24f, 3.18f)));
                    go.transform.localEulerAngles = new Vector3(24f, 0f, 0f);
                },
                (root) => { /* peak */ },
                Cost((woodLog, 3), (plank, 2)),
                Cost((stone, 5)),
            ...
            // PILLAR — full-height 3.75 m column.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.Pillar, "Pillar",
                (root, mat) => { AddBox(root, mat, ScaleConstruction(new Vector3(0f, 1.5f, 0f)), ScaleConstruction(new Vector3(0.44f, 3.0f, 0.44f))); },
                (root) => {
                    float halfModule = constructionModuleSize * 0.5f;
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, new Vector3(0f, constructionModuleSize, 0f), VoxelEngine.Building.Tiered.BuildFamily.Pillar);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.North, new Vector3(0f, 0f, halfModule), VoxelEngine.Building.Tiered.BuildFamily.Pillar);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.South, new Vector3(0f, 0f, -halfModule), VoxelEngine.Building.Tiered.BuildFamily.Pillar);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.East, new Vector3(halfModule, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Pillar);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.West, new Vector3(-halfModule, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.Pillar);
                },
            ...
            // HALF WALL — 3.75 m wide and 1.875 m tall.
            MakeFamily(VoxelEngine.Building.Tiered.BuildFamily.HalfWall, "HalfWall",
                (root, mat) => { AddBox(root, mat, ScaleConstruction(new Vector3(0f, 0.75f, 0f)), ScaleConstruction(new Vector3(3.0f, 1.5f, 0.24f))); },
                (root) => {
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.Top, new Vector3(0f, constructionModuleSize * 0.5f, 0f), VoxelEngine.Building.Tiered.BuildFamily.HalfWall);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.East, new Vector3(constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.HalfWall);
                    AddSocket(root, VoxelEngine.Building.Tiered.SocketSide.West, new Vector3(-constructionModuleSize, 0f, 0f), VoxelEngine.Building.Tiered.BuildFamily.HalfWall);
                },
                Cost((woodLog, 1), (plank, 1)),
                Cost((stone, 2)),
                Cost((ironIngot, 1)),
            ...
            foreach (var sys in systems)
            {
                ...
                sys.gridSize = constructionModuleSize;
                sys.socketSnapRadius = 3.25f;
        ...
        private static void AddSocket(GameObject parent,
            ...
            VoxelEngine.Building.Tiered.BuildFamily family, float localYaw = 0f)
        {
        ...
        private void BuildPowerContent()
        {
            ...
            const string pipesFolder    = powerFolder + "/ElectricalPipes";
            ...
            var graphite    = AssetDatabase.LoadAssetAtPath<VoxelEngine.Items.ResourceItem>($"{indItems}/Item_Graphite.asset");
            var stone       = AssetDatabase.LoadAssetAtPath<VoxelEngine.Items.ItemDefinition>($"{ITEM_FOLDER}/Item_Stone.asset");
            ...
            GameObject MakePowerPrefabInternal<T>(string assetName, Color color, Vector3 scale, System.Action<T> configure) where T : VoxelEngine.Power.PowerNode
            {
                ...
                return GetOrCreatePrefab(path, assetName, (root) =>
                {
                    if (root.transform.childCount == 0)
                    {
                        ...
                        cube.GetComponent<Renderer>().sharedMaterial = mat;
                        if (cube.TryGetComponent<Collider>(out var col)) Object.DestroyImmediate(col);
                    }
            ...
            // ── LV Manual Wires ──
            var lvWireCu = MakeResource(itemsFolder, "Copper LV Wire", new Color(0.85f, 0.45f, 0.20f), 100, VoxelEngine.Items.ResourceCategory.Component, uiCategory: "Wire");
            lvWireCu.itemId = "copper_lv_wire";
            lvWireCu.description = "Manual low-voltage wire item. Hold it and left-click two LV stations/connectors to link them; it is not an energy pipe block.";
            var lvWireAu = MakeResource(itemsFolder, "Gold LV Wire", new Color(0.95f, 0.85f, 0.20f), 100, VoxelEngine.Items.ResourceCategory.Component, uiCategory: "Wire");
            lvWireAu.itemId = "gold_lv_wire";
            lvWireAu.description = "Manual low-voltage wire item. Hold it and left-click two LV stations/connectors to link them; it is not an energy pipe block.";
            var lvWireGr = MakeResource(itemsFolder, "Graphite LV Wire", new Color(0.20f, 0.20f, 0.20f), 100, VoxelEngine.Items.ResourceCategory.Component, uiCategory: "Wire");
            lvWireGr.itemId = "graphite_lv_wire";
            lvWireGr.description = "Manual low-voltage wire item. Hold it and left-click two LV stations/connectors to link them; it is not an energy pipe block.";
        ...
        private void BuildResearchContent()
        {
            ...
            Lock(recPipeCu);   Lock(recPipeFe);  Lock(recPipeAu);  Lock(recPipeSc);
            Lock(recWireAuLV); Lock(recWireGrLV);
            ...
            var nAdvManufacturing = MakeNode("res_adv_manufacturing", "Advanced Manufacturing",
                "Industrial-tier production unlocks the Assembler, Electric Furnace, Battery, and mid-tier wiring.",
                3, 0, 60f, new[] { (sciT2, 15), (sciT3, 5) },
                new[] { recAssembler, recElectric, recBattery, recWireAuLV, recPipeAu },
        ...
        private void BuildIndustrialContent()
        {
            ...
            var copperWire  = MakeIndustrialResource("Item_CopperWire", "Copper LV Wire",   "Drawn copper conductor. The basic ingredient of all electronics.",            new Color(0.92f, 0.58f, 0.30f), VoxelEngine.Items.ResourceCategory.Component, "Electronics");
            ...
            var recIronPlate   = AddRecipe("Recipe_IronPlate", "Iron Plate", ironPlate, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironIngot, 2));
            var recCopperPlate = AddRecipe("Recipe_CopperPlate", "Copper Plate", copperPlate, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)copperIngot, 2));
            var recSteelPlate  = AddRecipe("Recipe_SteelPlate", "Steel Plate", steelPlate, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelIngot, 2));
            var recIronGear    = AddRecipe("Recipe_IronGear", "Iron Gear", ironGear, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2));

            // ── Wires & circuits (gated by Electronics / Adv Electronics) ──
            var recCopperWire  = AddRecipe("Recipe_CopperWire", "Copper Wire x2", copperWire, 2, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)copperIngot, 1));
            var recCircuit     = AddRecipe("Recipe_Circuit", "Electronic Circuit", circuitBasic, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 1), ((VoxelEngine.Items.ItemDefinition)copperWire, 3));
            ...
            var recWST = AddRecipe("Recipe_WirelessStorageTerminal", "Wireless Storage Terminal", blockWirelessST, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 4), (advCircuitOrFallback(circuitAdv, circuitBasic), 3), ((VoxelEngine.Items.ItemDefinition)copperWire, 8), ((VoxelEngine.Items.ItemDefinition)glass, 2));
            ...
            EditorUtility.DisplayDialog("Voxel Engine — Industrial Pack",
                ...
                "Open the new factory research Research UI in-game (key Y) to explore.", "OK");
        }
        ...
        private void BuildSurvivalAndLogisticsContent()
        {
            ...
            // ── Farming recipes ──
            AddRecipe(FARM_RECIPES, "Recipe_Hoe", "Hoe", hoe, 1, VoxelEngine.Crafting.StationTier.None, false, ((VoxelEngine.Items.ItemDefinition)woodLog, 2), ((VoxelEngine.Items.ItemDefinition)ironIngot, 1));
            AddRecipe(FARM_RECIPES, "Recipe_TilledSoil", "Tilled Soil", blockTilled, 1, VoxelEngine.Crafting.StationTier.None, false, ((VoxelEngine.Items.ItemDefinition)clay, 1));
            ...
            // Ruin prefabs — PREMIUM: realistic ruined crusader bases using real building blocks (walls, foundations)
            // that are mineable for resources, with rusted premium materials, collapsed sections, overgrowth.
            // FIXES from user feedback (image.png):
            // - Walls were scattered, not together → now exact module grid (3.75m), no random pos offset for base, only slight rotation tilt for ruined feel
            // - Chest invisible but lootable → now visible chest mesh + glow light
            // - Said steel/iron but looked wood → now uses steel prefabs if available and rusted metallic mats (0.62,0.32,0.18 orange rust, not wood brown)
            // - Too many spawns → now rarer (0.002, 0.0015, 0.0018) and easy numbers to tweak
            ...
            AddRecipe(MISC_RECIPES, "Recipe_Wrench", "Wrench", wrench, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)ironGear, 1));
            ...
            AddRecipe(MISC_RECIPES, "Recipe_PowerBusbar", "Power Busbar", blockBusbar, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)copperPlate, 4), ((VoxelEngine.Items.ItemDefinition)ironPlate, 1));
            ...
            // Sleeved industrial look: dark steel sleeve along the
            ...
            AddRecipe(MISC_RECIPES, "Recipe_ItemPipe", "Item Pipe x4", blockItemPipe, 4, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 1));
            AddRecipe(MISC_RECIPES, "Recipe_ItemPipe_Glass", "Item Pipe (Glass) x4", blockItemPipeGlass, 4, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 1), ((VoxelEngine.Items.ItemDefinition)glass, 1));
...
root =>
            ...
            AddRecipe(MISC_RECIPES, "Recipe_QuarryUpgradeSpeed", "Speed Upgrade", upgSpeed, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 4), ((VoxelEngine.Items.ItemDefinition)circuit, 3), ((VoxelEngine.Items.ItemDefinition)ironGear, 2));

            AddRecipe(MISC_RECIPES, "Recipe_QuarryUpgradeEfficiency", "Efficiency Upgrade", upgEff, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 6), ((VoxelEngine.Items.ItemDefinition)circuit, 6), ((VoxelEngine.Items.ItemDefinition)ironGear, 4), ((VoxelEngine.Items.ItemDefinition)copperWire, 2));
            ...
            var gasTankPrefab = MakePref(GAS_PREFABS, "GasTank",
                ...
                root =>
                {
                    ...
                    t.capacity = 10000f; t.acceptInput = true; t.allowOutput = true;
                });
            ...
            AddRecipe(GAS_RECIPES, "Recipe_Electrolyser", "Electrolyser", blockElectrolyser, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 4), ((VoxelEngine.Items.ItemDefinition)circuit, 3), ((VoxelEngine.Items.ItemDefinition)copperWire, 6), ((VoxelEngine.Items.ItemDefinition)glass, 2));
            AddRecipe(GAS_RECIPES, "Recipe_HydrogenEngine", "Hydrogen Engine", blockHydrogenEngine, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 4), ((VoxelEngine.Items.ItemDefinition)ironGear, 4), ((VoxelEngine.Items.ItemDefinition)circuit, 2));
            AddRecipe(GAS_RECIPES, "Recipe_GasTank", "Gas Tank", blockGasTank, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 3), ((VoxelEngine.Items.ItemDefinition)glass, 1));
            AddRecipe(GAS_RECIPES, "Recipe_GasPipe", "Gas Pipe x4", blockGasPipe, 4, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)copperPlate, 1));
            AddRecipe(GAS_RECIPES, "Recipe_GasPipe_Glass", "Gas Pipe (Glass) x4", blockGasPipeGlass, 4, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)copperPlate, 1), ((VoxelEngine.Items.ItemDefinition)glass, 1));
            ...
            var reactorPrefab = MakePref(NUKE_PREFABS, "ReactorCore",
                ...
                root =>
                {
                    ...
                    r.fuelRodBurnTime = 600f; r.maxThermalKW = 10000f;
            ...
            AddRecipe(NUKE_RECIPES, "Recipe_UraniumProcessor", "Uranium Processor", blockProcessor, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 10), ((VoxelEngine.Items.ItemDefinition)ironGear, 6), ((VoxelEngine.Items.ItemDefinition)advCircuit, 4), ((VoxelEngine.Items.ItemDefinition)copperWire, 8));
            AddRecipe(NUKE_RECIPES, "Recipe_ReactorCore", "Reactor Core", blockReactor, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 24), ((VoxelEngine.Items.ItemDefinition)ironGear, 12), ((VoxelEngine.Items.ItemDefinition)advCircuit, 8), ((VoxelEngine.Items.ItemDefinition)glass, 6), ((VoxelEngine.Items.ItemDefinition)copperWire, 16));
            AddRecipe(NUKE_RECIPES, "Recipe_SteamTurbine", "Steam Turbine", blockTurbine, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 16), ((VoxelEngine.Items.ItemDefinition)ironGear, 12), ((VoxelEngine.Items.ItemDefinition)advCircuit, 4), ((VoxelEngine.Items.ItemDefinition)copperWire, 8));
            AddRecipe(NUKE_RECIPES, "Recipe_PortableReactor", "Portable Reactor", blockPortable, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 6), ((VoxelEngine.Items.ItemDefinition)advCircuit, 3), ((VoxelEngine.Items.ItemDefinition)copperWire, 4));
            AddRecipe(NUKE_RECIPES, "Recipe_WasteReprocessor", "Waste Reprocessor", blockRepro, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 12), ((VoxelEngine.Items.ItemDefinition)ironGear, 8), ((VoxelEngine.Items.ItemDefinition)advCircuit, 6), ((VoxelEngine.Items.ItemDefinition)glass, 4));
            ...
            // Components
            AddRecipe(STORE_RECIPES, "Recipe_RAM_4", "RAM Module (4)", ram4, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)circuit, 1), ((VoxelEngine.Items.ItemDefinition)copperWire, 4));
            AddRecipe(STORE_RECIPES, "Recipe_RAM_16", "RAM Module (16)", ram16, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)advCircuit, 1), ((VoxelEngine.Items.ItemDefinition)copperWire, 4));
            AddRecipe(STORE_RECIPES, "Recipe_CPU_1", "CPU (1x)", cpu1, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)circuit, 2), ((VoxelEngine.Items.ItemDefinition)copperPlate, 1));
            AddRecipe(STORE_RECIPES, "Recipe_CPU_2", "CPU (2x)", cpu2, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)circuit, 4), ((VoxelEngine.Items.ItemDefinition)copperPlate, 2));
            AddRecipe(STORE_RECIPES, "Recipe_CPU_4", "CPU (4x)", cpu4, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)advCircuit, 2), ((VoxelEngine.Items.ItemDefinition)steelPlate, 1));
            AddRecipe(STORE_RECIPES, "Recipe_PSU_500", "PSU (500 W)", psu500, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)copperWire, 6));
            AddRecipe(STORE_RECIPES, "Recipe_PSU_2K", "PSU (2000 W)", psu2k, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 2), ((VoxelEngine.Items.ItemDefinition)copperWire, 8), ((VoxelEngine.Items.ItemDefinition)advCircuit, 1));

            // Disks
            AddRecipe(STORE_RECIPES, "Recipe_StorageDisk1K", "Storage Disk (1K)", disk1K, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 1), ((VoxelEngine.Items.ItemDefinition)circuit, 1));
            AddRecipe(STORE_RECIPES, "Recipe_StorageDisk4K", "Storage Disk (4K)", disk4K, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)circuit, 2));
            AddRecipe(STORE_RECIPES, "Recipe_StorageDisk16K", "Storage Disk (16K)", disk16K, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 1), ((VoxelEngine.Items.ItemDefinition)circuit, 4));
            AddRecipe(STORE_RECIPES, "Recipe_StorageDisk64K", "Storage Disk (64K)", disk64K, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 2), ((VoxelEngine.Items.ItemDefinition)advCircuit, 2), ((VoxelEngine.Items.ItemDefinition)plastic, 2));
            AddRecipe(STORE_RECIPES, "Recipe_StorageDisk90K", "Storage Disk (90K)", disk90K, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 3), ((VoxelEngine.Items.ItemDefinition)advCircuit, 4), ((VoxelEngine.Items.ItemDefinition)plastic, 4));

            // Blocks
            AddRecipe(STORE_RECIPES, "Recipe_ServerRack", "Server Rack", blockServerRack, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 6), ((VoxelEngine.Items.ItemDefinition)advCircuit, 4), ((VoxelEngine.Items.ItemDefinition)copperWire, 8));
            AddRecipe(STORE_RECIPES, "Recipe_StorageTerminal", "Storage Terminal", blockTerm, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 3), ((VoxelEngine.Items.ItemDefinition)circuit, 2), ((VoxelEngine.Items.ItemDefinition)glass, 1));
            AddRecipe(STORE_RECIPES, "Recipe_CraftingTerminal", "Crafting Terminal", blockCraftTerm, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 3), ((VoxelEngine.Items.ItemDefinition)circuit, 3), ((VoxelEngine.Items.ItemDefinition)glass, 1));
            AddRecipe(STORE_RECIPES, "Recipe_PatternTerminal", "Pattern Terminal", blockPatTerm, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 3), ((VoxelEngine.Items.ItemDefinition)advCircuit, 1), ((VoxelEngine.Items.ItemDefinition)glass, 1));
            AddRecipe(STORE_RECIPES, "Recipe_StorageImporter", "Storage Importer", blockImporter, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)ironGear, 2), ((VoxelEngine.Items.ItemDefinition)circuit, 1));
            AddRecipe(STORE_RECIPES, "Recipe_StorageExporter", "Storage Exporter", blockExporter, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)ironGear, 2), ((VoxelEngine.Items.ItemDefinition)circuit, 1));
            AddRecipe(STORE_RECIPES, "Recipe_NASBlock", "NAS Block", blockNAS, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 4), ((VoxelEngine.Items.ItemDefinition)circuit, 4), ((VoxelEngine.Items.ItemDefinition)copperWire, 6));
            AddRecipe(STORE_RECIPES, "Recipe_Powerstation", "Powerstation", blockPowerstation, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 4), ((VoxelEngine.Items.ItemDefinition)copperWire, 8), ((VoxelEngine.Items.ItemDefinition)circuit, 2));
            AddRecipe(STORE_RECIPES, "Recipe_DiskManipulator", "Disk Manipulator", blockDiskManip, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 4), ((VoxelEngine.Items.ItemDefinition)circuit, 2));
            AddRecipe(STORE_RECIPES, "Recipe_StorageDrawer", "Storage Drawer", blockDrawer, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 3), ((VoxelEngine.Items.ItemDefinition)ironGear, 1), ((VoxelEngine.Items.ItemDefinition)copperWire, 2));
            AddRecipe(STORE_RECIPES, "Recipe_StorageDrawerController", "Drawer Controller", blockDrawerController, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 4), ((VoxelEngine.Items.ItemDefinition)circuit, 2), ((VoxelEngine.Items.ItemDefinition)copperWire, 4));
            AddRecipe(STORE_RECIPES, "Recipe_StorageItemDisplay", "Storage Item Display", blockItemDisplay, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)glass, 1), ((VoxelEngine.Items.ItemDefinition)copperWire, 2));
            AddRecipe(STORE_RECIPES, "Recipe_DrawerUpgradeVoid", "Void Upgrade", drawerVoid, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)circuit, 1), ((VoxelEngine.Items.ItemDefinition)plastic, 1));
            AddRecipe(STORE_RECIPES, "Recipe_DrawerUpgradeStack1x", "Drawer Stack Upgrade (1x)", drawerStack1, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 1));
            AddRecipe(STORE_RECIPES, "Recipe_DrawerUpgradeStack2x", "Drawer Stack Upgrade (2x)", drawerStack2, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 1), ((VoxelEngine.Items.ItemDefinition)copperWire, 1));
            AddRecipe(STORE_RECIPES, "Recipe_DrawerUpgradeStack4x", "Drawer Stack Upgrade (4x)", drawerStack4, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)ironPlate, 2), ((VoxelEngine.Items.ItemDefinition)circuit, 1));
            AddRecipe(STORE_RECIPES, "Recipe_DrawerUpgradeStack8x", "Drawer Stack Upgrade (8x)", drawerStack8, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 1), ((VoxelEngine.Items.ItemDefinition)circuit, 2));
            AddRecipe(STORE_RECIPES, "Recipe_DrawerUpgradeStack16x", "Drawer Stack Upgrade (16x)", drawerStack16, 1, VoxelEngine.Crafting.StationTier.Assembler, false, ((VoxelEngine.Items.ItemDefinition)steelPlate, 2), ((VoxelEngine.Items.ItemDefinition)advCircuit, 1));
            ...
            EditorUtility.DisplayDialog("Voxel Engine — Step 11",
                ...
                "  • Wrench (universal network connector tool)\n" +
                "  • Power Busbar (clean cable-organization conduit)\n" +
                "  • Item Pipes (Solid + Glass) — transparent industrial sleeve style\n" +
                "  • Quarry + Upgrades (Range / Speed / Efficiency)\n" +
        ...
        private void BuildGridSystemContent()
        {
            ...
            MakeThruster("IonThruster_Small",  "Small Ion Thruster",         SzS, new Color(0.5f,0.3f,0.95f),  TIon,  430000f,  300f, 0f, (steelPlate, 2), (circuit, 3));
            ...
            var batSmallPref = MakeGPref<VoxelEngine.GridSystem.GridBattery>("Battery_Small", new Color(0.2f, 0.7f, 0.3f), new Vector3(0.5f, 0.5f, 0.5f),
                b => { b.capacityWh = 1000000f; b.maxDischargeRate = 30000f; });
            ...
            // -- Piston Block --
            var pistonPref = MakeGPref<VoxelEngine.GridSystem.GridPiston>("Piston_Large", new Color(0.6f, 0.6f, 0.6f), Vector3.one, p => {
                p.targetLength = 5f;
                p.extensionSpeed = 2f;
            });
            // Custom Visuals for Piston (Since GridBlockMeshBuilder is generic)
            var pistonRoot = PrefabUtility.LoadPrefabContents(AssetDatabase.GetAssetPath(pistonPref));
            GameObject basePart = GameObject.CreatePrimitive(PrimitiveType.Cube);
            basePart.name = "Base";
            basePart.transform.SetParent(pistonRoot.transform, false);
            basePart.transform.localScale = new Vector3(0.8f, 0.4f, 0.8f);
            basePart.GetComponent<Renderer>().sharedMaterial = MakeColoredMat(PREFABS + "/Mats", "Mat_PistonBase", new Color(0.4f, 0.4f, 0.4f));

            GameObject headPart = GameObject.CreatePrimitive(PrimitiveType.Cube);
            headPart.name = "Head";
            headPart.transform.SetParent(pistonRoot.transform, false);
            headPart.transform.localScale = new Vector3(0.9f, 0.2f, 0.9f);
            headPart.transform.localPosition = new Vector3(0, 0.2f, 0);
            headPart.GetComponent<Renderer>().sharedMaterial = MakeColoredMat(PREFABS + "/Mats", "Mat_PistonHead", new Color(0.7f, 0.7f, 0.7f));

            PrefabUtility.SaveAsPrefabAsset(pistonRoot, AssetDatabase.GetAssetPath(pistonPref));
            PrefabUtility.UnloadPrefabContents(pistonRoot);

            var itemPiston = MakeGItem("GItem_Piston", "Industrial Piston", Color.white, pistonPref, VoxelEngine.GridSystem.GridSize.Large, 400, 600);
            itemPiston.description = "A powerful hydraulic piston. Pushes and pulls grid blocks above it. Configurable length (0-20m) and speed.";
            AddGRecipe("Recipe_GPiston", "Industrial Piston", itemPiston, (steelPlate, 8), (ironPlate, 4), (copperWire, 8), (circuit, 2));
        ...
        // ============================================================
        //                STEP 14 - FLOODLIGHT CONTENT
        // ============================================================
        private void BuildFloodlightContent()
        {
            const string floodFolder    = ASSET_ROOT + "/Power/Floodlights";
            const string prefabsFolder  = floodFolder + "/Prefabs";
            const string itemsFolder    = floodFolder + "/Items";
            const string recipesFolder  = floodFolder + "/Recipes";

            EnsureFolder(floodFolder);
            EnsureFolder(prefabsFolder);
            EnsureFolder(itemsFolder);
            EnsureFolder(recipesFolder);
            ...
            var copperWire = AssetDatabase.LoadAssetAtPath<VoxelEngine.Items.ResourceItem>($"{commonItems}/Item_CopperWire.asset");
            ...
            // -- 1) Stationary Floodlight --
            string statPath = $"{prefabsFolder}/Floodlight_Stationary.prefab";
            var statRoot = new GameObject("Floodlight_Stationary");
            // Base pole
            var pole = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            pole.transform.SetParent(statRoot.transform, false);
            pole.transform.localScale = new Vector3(0.2f, 2f, 0.2f);
            pole.transform.localPosition = new Vector3(0, 1f, 0);
            pole.GetComponent<Renderer>().sharedMaterial = MakeColoredMat(prefabsFolder, "Mat_FloodPole", new Color(0.4f, 0.4f, 0.45f));
            // Head
            var head = GameObject.CreatePrimitive(PrimitiveType.Cube);
            head.transform.SetParent(statRoot.transform, false);
            head.transform.localScale = new Vector3(0.5f, 0.2f, 0.5f);
            head.transform.localPosition = new Vector3(0, 2f, 0);
            head.GetComponent<Renderer>().sharedMaterial = MakeColoredMat(prefabsFolder, "Mat_FloodHead", new Color(0.6f, 0.6f, 0.65f));
            // Light
            var lightGo = new GameObject("LightSource");
            lightGo.transform.SetParent(head.transform, false);
            lightGo.transform.localPosition = new Vector3(0, -0.11f, 0);
            ...
            var statFlood = statRoot.AddComponent<VoxelEngine.Power.Floodlight>();
            statFlood.lightComponent = light;
            var statCons = statRoot.AddComponent<VoxelEngine.Power.PowerConsumer>();
            statCons.wattsPerSecond = 50f;
            statCons.connectRadius = 1.8f;

            var statPrefab = PrefabUtility.SaveAsPrefabAsset(statRoot, statPath);
            Object.DestroyImmediate(statRoot);

            var statItem = MakeBlock(itemsFolder, "Block_Floodlight_Stationary", "Stationary Floodlight", new Color(0.7f, 0.7f, 0.75f), statPrefab, "Power");
            statItem.description = "A powerful outdoor light. Consumes 50 W. Only glows when powered.";

            // -- 2) Grid Floodlight --
            string gridPath = $"{prefabsFolder}/Floodlight_Grid.prefab";
            var gridRoot = GetOrCreatePrefab(gridPath, "Floodlight_Grid", (root) =>
            {
                VoxelEngine.GridSystem.GridBlockMeshBuilder.Build(root, VoxelEngine.GridSystem.GridBlockMeshBuilder.Style.Generic, VoxelEngine.GridSystem.GridSize.Small, new Color(0.6f, 0.6f, 0.65f));
                var lightGo = new GameObject("LightSource");
                lightGo.transform.SetParent(root.transform, false);
                lightGo.transform.localPosition = new Vector3(0, 0.51f, 0);
                var light = lightGo.AddComponent<Light>();
                light.type = LightType.Spot;
                light.intensity = 20f;
                light.range = 20f;
                ...
                var flood = root.AddComponent<VoxelEngine.Power.VoxelLightController>();
                flood.lightComponent = light;
                flood.intensity = 20f;
                var cons = root.AddComponent<VoxelEngine.Power.PowerConsumer>();
                cons.wattsPerSecond = 50f;
                cons.connectRadius = 1.6f;
            });

            var gridItem = MakeBlock(itemsFolder, "Block_Floodlight_Grid", "Grid Floodlight", new Color(0.7f, 0.7f, 0.75f), gridRoot, "Power");
            gridItem.description = "Grid-mountable floodlight. Consumes 50 W. Only glows when powered.";

            // -- 3) LED Strip Light --
            string ledPath = $"{prefabsFolder}/LEDStrip_Stationary.prefab";
            var ledRoot = new GameObject("LEDStrip_Stationary");
            var stripMesh = GameObject.CreatePrimitive(PrimitiveType.Cube);
            stripMesh.transform.SetParent(ledRoot.transform, false);
            stripMesh.transform.localScale = new Vector3(2f, 0.1f, 0.1f);
            stripMesh.GetComponent<Renderer>().sharedMaterial = MakeColoredMat(prefabsFolder, "Mat_LEDStrip", new Color(0.9f, 0.9f, 0.9f));

            var ledLightGo = new GameObject("LEDLight");
            ledLightGo.transform.SetParent(ledRoot.transform, false);
            ledLightGo.transform.localPosition = new Vector3(0, 0.1f, 0);
            var ledLight = ledLightGo.AddComponent<Light>();
            ledLight.type = LightType.Point;
            ledLight.range = 5f;
            ledLight.intensity = 5f;
            ledLight.color = Color.cyan;

            var ledCtrl = ledRoot.AddComponent<VoxelEngine.Power.VoxelLightController>();
            ledCtrl.lightComponent = ledLight;
            ledCtrl.lightColor = Color.cyan;
            ledCtrl.intensity = 5f;
            var ledCons = ledRoot.AddComponent<VoxelEngine.Power.PowerConsumer>();
            ledCons.wattsPerSecond = 10f;
            ledCons.connectRadius = 1.8f;

            var ledPrefab = PrefabUtility.SaveAsPrefabAsset(ledRoot, ledPath);
            Object.DestroyImmediate(ledRoot);

            var ledItem = MakeBlock(itemsFolder, "Block_LEDStrip", "LED Strip Light", new Color(0, 1, 1), ledPrefab, "Power");
            ledItem.description = "A thin LED strip. Low power (10 W). Supports color changes.";

            // -- 4) RGB Light --
            string rgbPath = $"{prefabsFolder}/RGB_Light.prefab";
            var rgbRoot = new GameObject("RGB_Light");
            var rgbMesh = GameObject.CreatePrimitive(PrimitiveType.Cube);
            rgbMesh.transform.SetParent(rgbRoot.transform, false);
            rgbMesh.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
            rgbMesh.GetComponent<Renderer>().sharedMaterial = MakeColoredMat(prefabsFolder, "Mat_RGB", Color.white);

            var rgbLightGo = new GameObject("RGBLight");
            rgbLightGo.transform.SetParent(rgbRoot.transform, false);
            var rgbLight = rgbLightGo.AddComponent<Light>();
            rgbLight.type = LightType.Point;
            rgbLight.range = 10f;
            rgbLight.intensity = 15f;
            rgbLight.color = Color.magenta;

            var rgbCtrl = rgbRoot.AddComponent<VoxelEngine.Power.VoxelLightController>();
            rgbCtrl.lightComponent = rgbLight;
            rgbCtrl.lightColor = Color.magenta;
            rgbCtrl.intensity = 15f;
            var rgbCons = rgbRoot.AddComponent<VoxelEngine.Power.PowerConsumer>();
            rgbCons.wattsPerSecond = 20f;
            rgbCons.connectRadius = 1.8f;

            var rgbPrefab = PrefabUtility.SaveAsPrefabAsset(rgbRoot, rgbPath);
            Object.DestroyImmediate(rgbRoot);

            var rgbItem = MakeBlock(itemsFolder, "Block_RGB_Light", "RGB Mood Light", new Color(1, 0, 1), rgbPrefab, "Power");
            rgbItem.description = "Small RGB light. Change colors via the Lighting UI. Consumes 20 W.";
            ...
            var recStat = AddRec("Recipe_Floodlight_Stationary", "Stationary Floodlight", statItem, (ironIngot, 2), (copperWire, 4), (glass, 1));
            var recGrid = AddRec("Recipe_Floodlight_Grid", "Grid Floodlight", gridItem, (ironIngot, 2), (copperWire, 4), (glass, 1));
            var recLed  = AddRec("Recipe_LEDStrip", "LED Strip Light", ledItem, (copperWire, 8), (glass, 2));
            var recRgb  = AddRec("Recipe_RGB_Light", "RGB Mood Light", rgbItem, (copperWire, 8), (glass, 1), (ironIngot, 1));
            ...
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            EditorUtility.DisplayDialog("Voxel Engine", "Lighting content created!\n\n- Floodlights, LED Strips, RGB Lights\n- Recipes\n- 'Advanced Lighting' research node", "OK");
        }
        ...
        // ============================================================
        // ============================================================
        //       STEP 17 - FACTORY FOUNDATIONS + HIGH VOLTAGE GRID
        // ============================================================
        private void BuildFactoryFoundationsContent()
        {
            ...
            var ironGear = AssetDatabase.LoadAssetAtPath<VoxelEngine.Items.ResourceItem>($"{indItems}/Item_IronGear.asset");
            var glass = AssetDatabase.LoadAssetAtPath<VoxelEngine.Items.ResourceItem>($"{indItems}/Item_Glass.asset");
            var plastic = AssetDatabase.LoadAssetAtPath<VoxelEngine.Items.ResourceItem>($"{indItems}/Item_Plastic.asset");
            ...
            VoxelEngine.Crafting.RecipeDefinition CreateRecipe(VoxelEngine.Crafting.RecipeRegistry recipeRegistry, string folder, string assetName, string display, VoxelEngine.Items.ItemDefinition output, int outputCount, VoxelEngine.Crafting.StationTier station, bool unlocked, params (VoxelEngine.Items.ItemDefinition item, int n)[] inputs)
            {
                ...
                if (created)
                {
                    ...
                    recipe.craftSeconds = station switch
                    {
                        VoxelEngine.Crafting.StationTier.None => 0f,
                        VoxelEngine.Crafting.StationTier.CraftingBench => 2f,
                        VoxelEngine.Crafting.StationTier.Furnace => 4f,
                        VoxelEngine.Crafting.StationTier.Assembler => 4f,
                        _ => 0f
                    };
            ...
            var conveyorBasicPrefab = CreateConveyorPrefab("Conveyor_Basic", VoxelEngine.Simulation.ConveyorSpeed.Basic, new Color(0.22f, 0.78f, 0.42f), 8);
            var conveyorFastPrefab = CreateConveyorPrefab("Conveyor_Fast", VoxelEngine.Simulation.ConveyorSpeed.Fast, new Color(0.18f, 0.72f, 0.88f), 10);
            var conveyorExpressPrefab = CreateConveyorPrefab("Conveyor_Express", VoxelEngine.Simulation.ConveyorSpeed.Express, new Color(0.95f, 0.62f, 0.18f), 12);

            var blockConveyorBasic = ConfigureBlock(FAC_ITEMS, "Block_ConveyorBasic", "Basic Conveyor Belt", "Moves item packets at 2 items per second. Snaps to other conveyors and machine ports.", new Color(0.22f, 0.78f, 0.42f), conveyorBasicPrefab, "Factory", 180);
            var blockConveyorFast = ConfigureBlock(FAC_ITEMS, "Block_ConveyorFast", "Fast Conveyor Belt", "Moves item packets at 5 items per second for denser factory lines.", new Color(0.18f, 0.72f, 0.88f), conveyorFastPrefab, "Factory", 200);
            ...
            var crushCopper = ConfigureMachineRecipe("MachineRecipe_CrushCopperOre", "Crush Copper Ore", VoxelEngine.Simulation.MachineRecipeType.Crushing, copperIngot, 2, 5f, false, gravel, 1, 0.25f, (copperOre, 1));

            var assembleGear = ConfigureMachineRecipe("MachineRecipe_AssembleGear", "Assemble Iron Gear", VoxelEngine.Simulation.MachineRecipeType.Assembling, ironGear, 1, 3f, false, null, 0, 0f, (ironPlate, 2));
            var assembleWire = ConfigureMachineRecipe("MachineRecipe_AssembleCopperWire", "Draw Copper Wire", VoxelEngine.Simulation.MachineRecipeType.Assembling, copperWire, 2, 2f, false, null, 0, 0f, (copperIngot, 1));
            var assembleCircuit = ConfigureMachineRecipe("MachineRecipe_AssembleCircuit", "Assemble Electronic Circuit", VoxelEngine.Simulation.MachineRecipeType.Assembling, circuit, 1, 5f, false, null, 0, 0f, (ironPlate, 1), (copperWire, 3));
            var assembleAdvCircuit = ConfigureMachineRecipe("MachineRecipe_AssembleAdvancedCircuit", "Assemble Advanced Circuit", VoxelEngine.Simulation.MachineRecipeType.Assembling, advCircuit, 1, 8f, false, null, 0, 0f, (circuit, 2), (plastic, 2), (copperWire, 4));
            ...
            var assemblerMk1Prefab = CreateAssemblerPrefab("Assembler_Mk1", VoxelEngine.Simulation.AssemblerTier.Mk1, new Color(0.18f, 0.72f, 0.88f), 300f, 10f);
            ...
            var smeltSteel = AssetDatabase.LoadAssetAtPath<VoxelEngine.Crafting.SmeltingRecipe>($"{ASSET_ROOT}/Recipes/Smelt_Steel.asset");
            var smeltGlass = AssetDatabase.LoadAssetAtPath<VoxelEngine.Crafting.SmeltingRecipe>($"{ASSET_ROOT}/Industrial/Recipes/Smelt_Glass.asset");
            var smeltingRecipes = new List<VoxelEngine.Crafting.SmeltingRecipe>();
            ...
            var electricFurnacePrefab = GetOrCreateStep17Prefab($"{FAC_PREFABS}/ElectricFurnace.prefab", "ElectricFurnace", root =>
            {
                ...
                EnsurePrimitive(root, "Generated_Shell", PrimitiveType.Cube, new Vector3(0f, 0.48f, 0f), new Vector3(1.15f, 0.95f, 1.15f), shellMat, Vector3.zero);
            ...
            ConfigureMachineDefinition("MachineDef_AssemblerMk2", "assembler_mk2", "Assembler Mk.2", "Faster component assembler with larger buffers.", VoxelEngine.Simulation.MachineCategory.Assembler, 450f, 15f, 1.5f, 6, 6, 3, new Color(0.28f, 0.25f, 0.20f), new Color(0.92f, 0.60f, 0.12f), assemblerMk2Prefab);
            ConfigureMachineDefinition("MachineDef_AssemblerMk3", "assembler_mk3", "Assembler Mk.3", "High-throughput component assembler for dense production lines.", VoxelEngine.Simulation.MachineCategory.Assembler, 700f, 22f, 2.5f, 9, 8, 4, new Color(0.26f, 0.22f, 0.30f), new Color(0.72f, 0.42f, 0.95f), assemblerMk3Prefab);
            ...
            GameObject CreateCompactPowerNodePrefab(string assetName, Color accent, System.Action<GameObject> addNode)
            {
                return GetOrCreateStep17Prefab($"{HV_PREFABS}/{assetName}.prefab", assetName, root =>
                {
                    ...
                    addNode?.Invoke(root);
                    var oldCable = root.GetComponent<VoxelEngine.Power.PowerCable>();
            ...
            var lvRelayPrefab = CreateCompactPowerNodePrefab("LVPowerRelay", new Color(0.95f, 0.62f, 0.18f), root =>
                EnsureStep17Component<VoxelEngine.Simulation.PowerRelayStation>(root));
            ...
            GameObject CreateGridDoorPrefab(string assetName, string displayName, string style)
            {
                return GetOrCreateStep17Prefab($"{GRID_PREFABS}/{assetName}.prefab", assetName, root =>
                {
                    // Door visuals are fully setup-generated; clear older generated door pieces so
                    // ugly legacy variants are replaced without touching custom/non-generated children.
                    ...
                    float z = -scale * 0.48f; // sit on the mounted face/edge of the large-grid cell
                    bool single = style == "Single";
                    bool vault = style == "Vault";
                    bool dual = style == "Double";

                    var frameMat = GetMaterial(GRID_MATS, "Mat_DoorDeepGraphiteFrame", new Color(0.025f, 0.030f, 0.036f));
                    var outerMat = GetMaterial(GRID_MATS, "Mat_DoorBrushedSilverOuter", new Color(0.58f, 0.64f, 0.68f));
                    var orangeMat = GetMaterial(GRID_MATS, "Mat_DoorBurntOrangePanel", new Color(0.72f, 0.28f, 0.16f));
                    var darkPanelMat = GetMaterial(GRID_MATS, "Mat_DoorBlackInsetPanel", new Color(0.055f, 0.060f, 0.070f));
                    var vaultMat = GetMaterial(GRID_MATS, "Mat_DoorVaultSteel", new Color(0.36f, 0.40f, 0.42f));
                    ...
                    EnsurePrimitive(root, "Generated_OuterFrameRight", PrimitiveType.Cube, new Vector3(0.58f * scale, 0f, z), new Vector3(0.08f * scale, 1.04f * scale, 0.12f * scale), outerMat, Vector3.zero);
                    EnsurePrimitive(root, "Generated_InnerSealTop", PrimitiveType.Cube, new Vector3(0f, 0.39f * scale, z - 0.018f * scale), new Vector3(0.96f * scale, 0.045f * scale, 0.08f * scale), frameMat, Vector3.zero);
                    EnsurePrimitive(root, "Generated_InnerSealBottom", PrimitiveType.Cube, new Vector3(0f, -0.39f * scale, z - 0.018f * scale), new Vector3(0.96f * scale, 0.045f * scale, 0.08f * scale), frameMat, Vector3.zero);
                    EnsurePrimitive(root, "Generated_InnerSealLeft", PrimitiveType.Cube, new Vector3(-0.48f * scale, 0f, z - 0.018f * scale), new Vector3(0.045f * scale, 0.82f * scale, 0.08f * scale), frameMat, Vector3.zero);
                    EnsurePrimitive(root, "Generated_InnerSealRight", PrimitiveType.Cube, new Vector3(0.48f * scale, 0f, z - 0.018f * scale), new Vector3(0.045f * scale, 0.82f * scale, 0.08f * scale), frameMat, Vector3.zero);
                    ...
                    if (vault)
                    {
                        ...
                        foreach (var x in new[] { -0.38f, 0.38f }) foreach (var y in new[] { -0.32f, 0.32f })
                            EnsurePrimitive(root, "Generated_VaultBolt_" + x + "_" + y, PrimitiveType.Sphere, new Vector3(x * scale, y * scale, z - 0.12f * scale), Vector3.one * 0.045f * scale, outerMat, Vector3.zero);
                    }
                    else if (single)
                    {
                    ...
                    EnsureStep17Component<VoxelEngine.GridSystem.GridSlidingDoor>(root, door =>
                    {
                        ...
                        door.motionGraceSeconds = vault ? 2.0f : 1.5f;
                        door.idleWatts = vault ? 8f : 4f;
                        door.movingWatts = vault ? 90f : 35f;
                    });
                });
            }
            ...
            var recConveyorFast = CreateRecipe(registry, FAC_RECIPES, "Recipe_ConveyorFast", "Fast Conveyor Belt", blockConveyorFast, 2, VoxelEngine.Crafting.StationTier.Assembler, false, (ironPlate, 3), (ironGear, 2), (copperWire, 2));
            ...
            {
                registry.recipes.Remove(legacySmallDoorRecipe);
                repairedLinkCount++;
            }
            ...
            var recLVPowerRelay = CreateRecipe(registry, HV_RECIPES, "Recipe_LVPowerRelay", "LV Power Relay", blockLVPowerRelay, 1, VoxelEngine.Crafting.StationTier.CraftingBench, false, (ironPlate, 2), (copperWire, 4), (circuit, 1));
            var recHVPowerRelay = CreateRecipe(registry, HV_RECIPES, "Recipe_HVPowerRelay", "HV Power Relay", blockHVPowerRelay, 1, VoxelEngine.Crafting.StationTier.Assembler, false, (steelPlate, 2), (copperWire, 8), (circuit, 2));
            ...
            foreach (var recipe in new[] { recHVWire, recPowerPole, recLVConnector, recHVConnector, recLVPowerRelay, recHVPowerRelay, recSubstation, recHVTower, recStepUp, recStepDown })
            {
            ...
            AddRecipeUnique(hvRecipes, recWireGrLV);
            AddRecipeUnique(hvRecipes, recPipeSc);
        ...
        private void BuildCelestialRuinsContent()
        {
            ...
            GameObject MakeCelestialRuin(string name, string theme, string archetype, int extent, int minComp, int maxComp)
            {
                ...
                // ── Per-variant architecture flags (every ruin gets a distinct silhouette) ──
                float  module        = 3.75f;
                bool   wantTall      = name.Contains("Tower") || name.Contains("Spire") || name.Contains("Citadel");
                bool   wantLow       = name.Contains("Bunker") || name.Contains("Cache") || name.Contains("Vault");
                bool   wantHeavyRuin = name.Contains("Wreck") || name.Contains("Camp") || name.Contains("Dissolved") || name.Contains("Dry") || name.Contains("Ash") || name.Contains("Frontier");
                bool   wantStilts    = name.Contains("Stilt") || name.Contains("Platform");
                bool   wantSunken    = name.Contains("Sunken");
                bool   wantForge     = name.Contains("Forge") || name.Contains("Refinery");
                bool   wantAntenna   = name.Contains("Listening") || name.Contains("Post") || name.Contains("Station");
                bool   wantAltar     = name.Contains("Shrine") || name.Contains("Temple") || name.Contains("Oracle") || name.Contains("Treasury");
                bool   wantShipwreck = name.Contains("Wreck") || name.Contains("Camp");
                bool   wantBanner    = (archetype == "Fortress") && !wantLow && UnityEngine.Random.value < 0.6f;
                bool   wantColumns   = (archetype == "Fortress") && extent >= 2 && !wantLow && UnityEngine.Random.value < 0.6f;

                float  W             = extent * module;
                float  footW         = (extent * 2 + 1) * module;
                float  wallHeight    = (wantLow ? 0.6f : wantTall ? 1.3f : 1.0f) * module;
                float  ruinChance    = wantHeavyRuin ? 0.30f : 0.13f;
                ...
                { var go = AddBlock(childName, localPos, localScale, mat, hp); go.transform.localRotation = rot; return go; }
                GameObject AddDecor(string childName, Vector3 localPos, Vector3 localScale, Material mat, Quaternion rot)
                {
                    var go = GameObject.CreatePrimitive(PrimitiveType.Cube);
                    go.name = childName;
                ...
                if (archetype == "Temple")
                {
                    ...
                    void CapRow(string side, float edge, bool alongX)
                    {
                        int lo = alongX ? -extent : -extent + 1, hi = alongX ? extent : extent - 1;
                        for (int i = lo; i <= hi; i++)
                        {
                    ...
                    CapRow("N", W, true); CapRow("S", -W, true); CapRow("E", W, false); CapRow("W", -W, false);
                    ...
                    float pedBase = entY + 0.20f, pedHalf = footW * 0.5f;
                ...
                else if (archetype == "Dome")
                {
                    ...
                    AddBlock("Airlock_L", new Vector3(-0.9f, ringY, W), new Vector3(0.3f, wallHeight, 0.5f), M["primaryDark"], 280);
                    AddBlock("Airlock_R", new Vector3(0.9f, ringY, W), new Vector3(0.3f, wallHeight, 0.5f), M["primaryDark"], 280);
                    ...
                    for (int i = 0; i < capCount; i++)
                    {
                        ...
                        Quaternion tilt = wantHeavyRuin ? Quaternion.Euler(UnityEngine.Random.Range(-6f, 6f), 0, UnityEngine.Random.Range(-6f, 6f)) : Quaternion.identity;
                        AddPrim($"DomeCap_{i}", PrimitiveType.Sphere, new Vector3(0, domeBase + i * 0.5f, 0), new Vector3(domeRadius * frac, domeRadius * frac * 0.55f, domeRadius * frac), M["primary"], 280, tilt);
                    }
                    ...
                    if (wantAntenna) // comms array: extra masts
                        for (int i = 0; i < 3; i++)
                            AddPrim($"CommMast_{i}", PrimitiveType.Cylinder, new Vector3(-W * 0.4f + i * 0.5f, domeBase + wallHeight * 0.5f, W * 0.4f), new Vector3(0.04f, wallHeight * 0.9f, 0.04f), M["accent"], 140, Quaternion.Euler(0, 0, UnityEngine.Random.Range(-4f, 4f)));
                    if (theme == "Venus")
                        ...
                        for (int i = 0; i < 4; i++) { float a = i / 4f * Mathf.PI * 2f; AddPrim($"IceSpike_{i}", PrimitiveType.Cube, new Vector3(Mathf.Cos(a) * domeRadius * 0.7f, domeBase + domeRadius * 0.3f, Mathf.Sin(a) * domeRadius * 0.7f), new Vector3(0.3f, 1.4f, 0.3f), M["accent"], 120, Quaternion.Euler(UnityEngine.Random.Range(-10f, 10f), 0f, UnityEngine.Random.Range(-10f, 10f))); }
                    else if (theme == "Crystal")
                        for (int i = 0; i < 5; i++) { float a = i / 5f * Mathf.PI * 2f; AddPrim($"GeodeCluster_{i}", PrimitiveType.Cube, new Vector3(Mathf.Cos(a) * domeRadius * 0.8f, domeBase + domeRadius * 0.4f, Mathf.Sin(a) * domeRadius * 0.8f), new Vector3(0.34f, 1.0f, 0.34f), M["accent"], 120, Quaternion.Euler(UnityEngine.Random.Range(-15f, 15f), UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(-15f, 15f))); }
                ...
                else // Fortress (Mars / Acid / Pirate / Ice / Water / Desolate / Volcanic / Crystal)
                {
                    float wallY = deckTopY + wallHeight * 0.5f;
                    ...
                    {
                        Vector3 full = alongX ? new Vector3(module * 0.99f, wallHeight, wallThick) : new Vector3(wallThick, wallHeight, module * 0.99f);
                        if (UnityEngine.Random.value < ruinChance)
                        {
                            Vector3 sill = alongX ? new Vector3(module * 0.99f, wallHeight * 0.34f, wallThick) : new Vector3(wallThick, wallHeight * 0.34f, module * 0.99f);
                    ...
                    bool collapsedSouth = wantHeavyRuin;
                    for (int xi = -extent; xi <= extent; xi++)
                    {
                    ...
                    if (!wantLow) // battlements
                    {
                        ...
                        {
                            if (xi == 0) continue;
                            if (((xi + 1000) & 1) == 0 && UnityEngine.Random.value > 0.2f) AddBlock($"Merlon_N{xi}", new Vector3(xi * module, mY, W), new Vector3(mW, mH, wallThick * 1.4f), PM(), 200);
                            if (((xi + 1001) & 1) == 0 && UnityEngine.Random.value > 0.2f) AddBlock($"Merlon_S{xi}", new Vector3(xi * module, mY, -W), new Vector3(mW, mH, wallThick * 1.4f), PM(), 200);
                        }
                    }

                    // Theme signature accents (unchanged flavour)
                    if (theme == "Mars")
                        for (int i = 0; i < 4; i++) { float dx = UnityEngine.Random.Range(-W * 0.8f, W * 0.8f); AddBlockRot($"DustDrift_{i}", new Vector3(dx, 0.35f, -W - 0.6f), new Vector3(module * 0.8f, 0.7f, 1.4f), M["stone"], 80, Quaternion.Euler(0, UnityEngine.Random.Range(0, 360f), -18f)); }
                    else if (theme == "Acid")
                    {
                        for (int i = 0; i < 5; i++) { float a = i / 5f * Mathf.PI * 2f, rx = Mathf.Cos(a) * (W + 0.6f), rz = Mathf.Sin(a) * (W + 0.6f), sh = UnityEngine.Random.Range(1.6f, 3.2f); AddPrim($"Crystal_{i}", PrimitiveType.Cube, new Vector3(rx, deckTopY + sh * 0.5f, rz), new Vector3(0.3f, sh, 0.3f), M["accent"], 120, Quaternion.Euler(UnityEngine.Random.Range(-12f, 12f), UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(-12f, 12f))); }
                        for (int i = 0; i < 4; i++) AddDecor($"SlimeDrip_{i}", new Vector3(UnityEngine.Random.Range(-W, W), deckTopY + wallHeight * 0.5f, W), new Vector3(0.08f, wallHeight * 0.8f, 0.08f), M["accent"], Quaternion.identity);
                    }
                    else if (theme == "Pirate")
                    {
                        ...
                        for (int i = 0; i < 5; i++) AddBlockRot($"Junk_{i}", new Vector3(UnityEngine.Random.Range(-W, W), deckTopY + 0.3f, UnityEngine.Random.Range(-W, W)), new Vector3(0.6f, 0.6f, 0.6f), PM(), 80, Quaternion.Euler(UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(0, 360f)));
                    }
                    else if (theme == "Ice")
                        for (int i = 0; i < 5; i++) AddDecor($"Icicle_{i}", new Vector3(UnityEngine.Random.Range(-W, W), deckTopY + wallHeight * 0.45f, UnityEngine.Random.Range(-W, W)), new Vector3(0.12f, wallHeight * 0.5f, 0.12f), M["primaryLight"], Quaternion.identity);
                    else if (theme == "Volcanic")
                    {
                        for (int i = 0; i < 3; i++) AddDecor($"MagmaVent_{i}", new Vector3(UnityEngine.Random.Range(-W * 0.6f, W * 0.6f), deckTopY + 0.05f, UnityEngine.Random.Range(-W * 0.6f, W * 0.6f)), new Vector3(0.8f, 0.08f, 0.8f), M["accent"], Quaternion.identity);
                        ...
                        for (int i = 0; i < 5; i++) { float a = i / 5f * Mathf.PI * 2f, sh = UnityEngine.Random.Range(1.6f, 3.0f); AddPrim($"Geode_{i}", PrimitiveType.Cube, new Vector3(Mathf.Cos(a) * (W + 0.5f), deckTopY + sh * 0.5f, Mathf.Sin(a) * (W + 0.5f)), new Vector3(0.3f, sh, 0.3f), M["accent"], 120, Quaternion.Euler(UnityEngine.Random.Range(-12f, 12f), UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(-12f, 12f))); }
                    else if (theme == "Water")
                    ...
                    if (collapsedSouth)
                        for (int i = 0; i < 4; i++) { float cs = UnityEngine.Random.Range(0.5f, 1.0f); AddBlockRot($"Collapse_{i}", new Vector3(UnityEngine.Random.Range(-module, module), 0.3f, -W - UnityEngine.Random.Range(0.3f, 1.7f)), new Vector3(cs, cs * 0.8f, cs), PM(), 120, Quaternion.Euler(UnityEngine.Random.Range(-25f, 25f), UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(-25f, 25f))); }

                    // Interior columns (some forts) — a few standing, a few toppled
                    if (wantColumns)
                    {
                        float colR2 = module * 0.7f, colH2 = wallHeight * 0.8f; int spot = 0;
                        foreach (float cx in new float[] { -colR2, colR2 }) foreach (float cz in new float[] { -colR2, colR2 })
                        {
                            if (UnityEngine.Random.value < 0.5f) { AddBlock($"ColBase_{spot}", new Vector3(cx, deckTopY + 0.1f, cz), new Vector3(0.6f, 0.2f, 0.6f), M["stoneDark"], 200); AddBlock($"ColShaft_{spot}", new Vector3(cx, deckTopY + 0.2f + colH2 * 0.5f, cz), new Vector3(0.44f, colH2, 0.44f), M["stone"], 180); }
                            else { AddBlockRot($"ColFallen_{spot}", new Vector3(cx + (cx < 0 ? -0.6f : 0.6f), deckTopY + 0.28f, cz + (cz < 0 ? -0.6f : 0.6f)), new Vector3(0.44f, 0.44f, colH2), M["stone"], 180, Quaternion.Euler(0f, UnityEngine.Random.Range(0, 360f), 90f)); AddBlock($"ColStub_{spot}", new Vector3(cx, deckTopY + 0.25f, cz), new Vector3(0.5f, 0.5f, 0.5f), M["stoneDark"], 180); }
                            spot++;
                        }
                    }
                    ...
                    {
                        float bx = -W + pillarW * 0.5f, bz = -W + pillarW * 0.5f;
                        AddBlock("BannerPole", new Vector3(bx, deckTopY + wallHeight * 0.5f, bz), new Vector3(0.12f, wallHeight, 0.12f), M["primaryDark"], 120);
                        var cloth = AddDecor("BannerCloth", new Vector3(bx + 0.4f, deckTopY + wallHeight * 0.74f, bz), new Vector3(0.8f, wallHeight * 0.52f, 0.04f), M["accent"], Quaternion.identity);
                        cloth.transform.localRotation = Quaternion.Euler(0f, 90f, UnityEngine.Random.Range(-4f, 4f));
                    }
                }

                // ── Per-variant side/centerpiece features (NEW: silhouettes you can tell apart) ──
                if (wantForge) // industrial chimney stack at a back corner
                {
                    float chx = -W + module * 0.5f, chz = -W + module * 0.5f, chH = wallHeight * 1.6f;
                    AddPrim("Chimney", PrimitiveType.Cylinder, new Vector3(chx, deckTopY + chH * 0.5f, chz), new Vector3(0.5f, chH * 0.5f, 0.5f), M["primaryDark"], 200, Quaternion.identity);
                    AddBlock("ChimneyCap", new Vector3(chx, deckTopY + chH, chz), new Vector3(0.7f, 0.25f, 0.7f), M["stoneDark"], 160);
                    AddDecor("ChimneyGlow", new Vector3(chx, deckTopY + chH * 0.25f, chz), new Vector3(0.4f, 0.4f, 0.4f), M["accent"], Quaternion.identity);
                    buildHeight = Mathf.Max(buildHeight, chH + deckTopY);
                }
                if (wantAntenna && archetype != "Dome") // comms mast (domes add their own array above)
                {
                    float ax = W - module * 0.5f, az = -W + module * 0.5f, aH = wallHeight * 2.0f;
                    AddPrim("CommMast", PrimitiveType.Cylinder, new Vector3(ax, deckTopY + aH * 0.5f, az), new Vector3(0.06f, aH * 0.5f, 0.06f), M["accent"], 140, Quaternion.Euler(0, 0, UnityEngine.Random.Range(-3f, 3f)));
                    AddDecor("CommDish", new Vector3(ax, deckTopY + aH * 0.8f, az), new Vector3(0.5f, 0.08f, 0.5f), M["primaryLight"], Quaternion.Euler(-30f, 0, 0));
                    buildHeight = Mathf.Max(buildHeight, aH + deckTopY);
                }
                if (wantShipwreck) // crashed hull beside the compound
                {
                    float hx = W + module * 1.1f, hz = 0f;
                    AddBlockRot("HullKeel", new Vector3(hx, 0.6f, hz), new Vector3(0.6f, 0.6f, module * 2.2f), M["primary"], 160, Quaternion.Euler(0, UnityEngine.Random.Range(-20f, 20f), 22f));
                    AddBlockRot("HullDeck", new Vector3(hx, 1.2f, hz), new Vector3(1.0f, 0.2f, module * 2.0f), M["primaryDark"], 140, Quaternion.Euler(0, UnityEngine.Random.Range(-20f, 20f), 22f));
                    AddPrim("HullNose", PrimitiveType.Sphere, new Vector3(hx, 0.9f, hz + module * 1.1f), new Vector3(0.7f, 0.6f, 0.9f), M["primary"], 140, Quaternion.Euler(0, UnityEngine.Random.Range(-20f, 20f), 22f));
                }

                // ── Central dais / altar + relic chest ──
                if (wantAltar)
                {
                    AddBlock("Altar", new Vector3(0, deckTopY + 0.25f, 0), new Vector3(1.8f, 0.5f, 1.8f), M["stone"], 300);
                    // Cult statue behind the chest
                    AddPrim("StatueBase", PrimitiveType.Cube, new Vector3(0, deckTopY + 0.55f, -0.9f), new Vector3(0.5f, 0.2f, 0.5f), M["stoneDark"], 200, Quaternion.identity);
                    AddPrim("StatueBody", PrimitiveType.Cylinder, new Vector3(0, deckTopY + 1.2f, -0.9f), new Vector3(0.22f, 0.55f, 0.22f), M["accent"], 160, Quaternion.identity);
                    AddPrim("StatueHead", PrimitiveType.Sphere, new Vector3(0, deckTopY + 1.85f, -0.9f), new Vector3(0.26f, 0.26f, 0.26f), M["accent"], 140, Quaternion.identity);
                }
                else
                {
                ...
                // Rubble (scales with ruin level)
                int rubbleN = wantLow ? 3 : (wantHeavyRuin ? 9 : 6);
                for (int i = 0; i < rubbleN; i++)
                {
                    ...
                    float rs = UnityEngine.Random.Range(0.22f, 0.55f), rr = UnityEngine.Random.value;
                    Material rm = rr < 0.4f ? M["stone"] : rr < 0.7f ? M["primaryDark"] : M["accent"];
                    AddBlockRot($"Rubble_{i}", new Vector3(rxx, deckTopY + rs * 0.4f + 0.04f, rzz), new Vector3(rs, rs * 0.8f, rs), rm, 80, Quaternion.Euler(UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(0, 360f), UnityEngine.Random.Range(0, 360f)));
                }

                float chestY = deckTopY + (wantAltar ? 0.55f : 0.45f);
                var chestGO = new GameObject("RuinChest_Visible");
                chestGO.transform.SetParent(root.transform, false);
                chestGO.transform.localPosition = new Vector3(0, chestY, 0);
                chestGO.transform.localScale = new Vector3(1.0f, 0.7f, 0.8f);
                {
        ...
        // ============================================================
        //   STEP 21 - CELESTIAL WORLDS (Phase 2: planets + themed biomes + ruin spawning)
        //   Wires every Phase-1 themed ruin to its own planet so it spawns in-world.
        //   Non-destructive: appends planets to Sol, preserves hand-tuned biome scatter.
        // ============================================================
        private void BuildCelestialWorldsContent()
        {
            const string PLANETS_DIR = ASSET_ROOT + "/Planets";
            const string BIOMES_DIR  = ASSET_ROOT + "/Biomes";
            const string RUINS_DIR   = ASSET_ROOT + "/Survival/CelestialRuins";
            EnsureFolder(PLANETS_DIR);
            EnsureFolder(BIOMES_DIR);
